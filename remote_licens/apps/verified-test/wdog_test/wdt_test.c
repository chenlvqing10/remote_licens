...
void *feeddogthread()
{
	...
	while (1) {
		...
		if (count%16 == 0 && watchdogfd > 0) {
			ioctl(watchdogfd, WDIOC_KEEPALIVE, &feeddogvalue);
			count = 0;
		}
		sleep(1);
	}
...
int main()
{
	...
	while (1) {
		...
		printf("Press 's' to set watchdog timeout in 10s!\n");
		...
		if (ch == 's') {
			printf("Set watchdog timeout in 10s!\n");
			tmp = 10;
