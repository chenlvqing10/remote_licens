...
int pcm_set_config(struct pcm *pcm, const struct pcm_config *config)
{
	...
	pcm->config.period_size =
		param_get_int(&params, SNDRV_PCM_HW_PARAM_PERIOD_SIZE);
	pcm->config.period_count =
		param_get_int(&params, SNDRV_PCM_HW_PARAM_PERIODS);
 ...
 * The clock is CLOCK_MONOTONIC if flag @ref PCM_MONOTONIC was specified in
 * @ref pcm_open,
 * otherwise the clock is CLOCK_REALTIME.
 * For an input stream, frames available are frames ready for the application
 * to read.
			...
			struct timespec *tstamp)
{
	...
	rc = pcm_sync_ptr(pcm,
		SNDRV_PCM_SYNC_PTR_APPL|SNDRV_PCM_SYNC_PTR_HWSYNC);
	if (rc < 0)
		return -1;
 ...
 *  or INT_MAX.
 * @return On success, this function returns the number of frames written;
	...
	for (;;) {
		if (!pcm->running) {
			...
			if (ioctl(pcm->fd, SNDRV_PCM_IOCTL_WRITEI_FRAMES, &x))
				return oops(pcm, errno,
					"cannot write initial data");
			pcm->running = 1;
			return 0;
		}
		if (ioctl(pcm->fd, SNDRV_PCM_IOCTL_WRITEI_FRAMES, &x)) {
			...
			pcm->running = 0;
			if (errno == EPIPE) {
				/* we failed to make our window --
				 * try to restart if we are allowed to do so.
				 * Otherwise, simply allow the EPIPE
				 * error to
				...
				enum pcm_param param)
{
	const struct snd_pcm_hw_params *params =
		(const struct snd_pcm_hw_params *)pcm_params;
		int p;
 ...
 *  Otherwise, client code should check that the PCM opened properly by
 * calling @ref pcm_is_ready.
 ...
 * @param pcm A PCM handle.
 *  May be NULL.
 * @return If a PCM's file descriptor is not valid or the pointer is NULL,
 * it returns zero.
...
/** Links two PCMs.
 * After this function is called, the two PCMs will prepare, start and stop in
 ...
 * @param pcm A PCM handle.
 * @param timeout The maximum amount of time to wait for, in terms of
 * milliseconds.
...
int pcm_mmap_transfer(struct pcm *pcm, const void *buffer, unsigned int bytes)
{
	...
	while (count > 0) {
		...
		if (avail < 0) {
			fprintf(stderr,
				"cannot determine available mmap frames");
			return err;
		}

		/* start the audio if we reach the threshold */
		if (!pcm->running &&
			(pcm->buffer_size - avail) >=
				pcm->config.start_threshold) {
			if (pcm_start(pcm) < 0) {
				fprintf(stderr,
		...
		if (pcm->running &&
			...
			if (pcm->flags & PCM_NOIRQ)
				time = (pcm->buffer_size - avail -
					pcm->mmap_control->avail_min)
		...
		frames = pcm_mmap_transfer_areas(pcm,
			(void *)buffer, offset, frames);
		if (frames < 0) {
			fprintf(stderr,
