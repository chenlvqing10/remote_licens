...
int dbname_query_result_count(char *db_name, const char *sql)
{
	...
	ret = dbname_query_by_var_param(db_name, sql, db_get_count, &count, NULL);
	if (SQLITE_OK == ret)
		return count;
	...
	const char *sql, char *name)
{
	...
	ret = dbname_query_by_var_param(db_name, sql, db_get_count, &count, "%s", name);
	if (SQLITE_OK == ret)
		return count;
 ...
 * Returns number of data queried.
 */
double dbname_query_result_min_punchtime(const char *db_name, const char *sql)
{
	int ret = 0;
	double count = 0;
	...
	ret = dbname_query_by_var_param(db_name, sql, db_get_min_punchtime, &count, NULL);
	if (SQLITE_OK == ret)
		return count;
	...
	int (*create)(sqlite3_stmt *stmt, void *arg), void *arg, const char *fmt, ...)
{
	...
	ALOGD("[%s:%d]main_db=%s, attached_db=%s, alias=%s\n", __func__, __LINE__,
		main_db_name, attached_db_name, attached_db_alias);
	ALOGD("[%s:%d] sql=%s\n", __func__, __LINE__, sql);

	pthread_mutex_lock(&db_mutex_lock);

	/* Open an SQLite database */
	int rc = sqlite3_open(main_db_name, &db);
	if (rc != SQLITE_OK) {
		ALOGE("[%s:%d] open database failed,rc=%d\n", __func__, __LINE__, rc);
		pthread_mutex_unlock(&db_mutex_lock);
	...
	snprintf(buf, sizeof(buf), "attach database '%s' as '%s';", attached_db_name,
		attached_db_alias);
	rc = sqlite3_exec(db, buf, 0, 0, NULL);
	if (rc != SQLITE_OK) {
		ALOGE("[%s:%d] attach database:ret=%d\n", __func__, __LINE__, rc);
		goto DB_EXEC_FAIL;
	}
	...
	if (create) {
		rc = (*create)(stmt, arg);
		ALOGD("[%s:%d]create rc=%d\n", __func__, __LINE__, rc);
	} else {
		rc = (sqlite3_step(stmt), 0);
		ALOGD("[%s:%d]sqlite3_step rc=%d\n", __func__, __LINE__, rc);
	}
	sqlite3_finalize(stmt);
...
DB_EXEC_OK:
	memset(buf, 0, sizeof(buf));
	snprintf(buf, sizeof(buf), "detach database '%s';", attached_db_alias);
	ret_exec = sqlite3_exec(db, buf, 0, 0, NULL);
	if (ret_exec != SQLITE_OK)
		ALOGE("[%s:%d] detach database:ret=%d\n", __func__, __LINE__, ret_exec);
	...
	int (*bind)(sqlite3_stmt *, unsigned int index, void *arg), void *param)
{
	...
	ALOGD("[%s]attach database ok; rc=%d\n", __func__, rc);
