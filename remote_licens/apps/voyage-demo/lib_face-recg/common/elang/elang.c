...
char *elang_str(const char *json_str)
{
	...
	str_index = 0;
	offset = 0;
	while (strcmp(json_str, (const char *)(g_lang->str_map + offset)) != 0) {
		offset += (strlen((const char *)(g_lang->str_map + offset))+1);
		str_index++;
...
void elang_test(void)
{
	...
	if (-1 == elang_load("lang.el"))
		return;

	elang_set_language(ELANG_ENGLISH);

	for (i = 0; i < ESTR_LANGUAGE_MAX; i++)
		printf("%s\n", elang_get_utf8_string(i));

	printf("%s\n", elang_str("STR_MONDAY"));
	printf("%s\n", elang_str("STR_TUESDAY"));
	printf("%s\n", elang_str("STR_WEDNESDAY"));
	printf("%s\n", elang_str("STR_THURSDAY"));
	printf("%s\n", elang_str("STR_FRIDAY"));
	printf("%s\n", elang_str("STR_SATURDAY"));
	printf("%s\n", elang_str("STR_SUNDAY"));
	...
	printf("%s\n", elang_str("STR_TAB_INPUT"));
	printf("%s\n", elang_str("STR_TAB_LIST"));
	printf("%s\n", elang_str("STR_TAB_CHART"));
	printf("%s\n", elang_str("STR_TAB_Setting"));
	printf("%s\n", elang_str("STR_TAB_lang"));
