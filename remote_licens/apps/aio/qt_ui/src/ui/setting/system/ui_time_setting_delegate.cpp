...
ui_time_setting_delegate::ui_time_setting_delegate() {
}
void ui_time_setting_delegate::paint(
  QPainter* painter,
  ...
  if (index.isValid()) {
    ...
    data_time_setting_delegate data = var.value<data_time_setting_delegate>();

    if (data.is_display == false) {
      painter->restore();
      return;
    }
    ...
    if (index.row() == 0) {
      data.widget->setGeometry(rect.x() +
      rect.width() - 72 - 102, rect.y() + 17, 102, 62);
    } else if (index.row() == 1) {
      data.widget->setGeometry(rect.x() +
      rect.width() - 72 - 360, rect.y() + 17, 360, 62);
    }
...
QSize ui_time_setting_delegate::sizeHint(
  const QStyleOptionViewItem& option,
  const QModelIndex& index) const {
  Q_UNUSED(index)
