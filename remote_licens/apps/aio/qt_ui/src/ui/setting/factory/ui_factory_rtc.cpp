/*
* ui_factory_rtc.cpp - define factory test rtc interface function of UI
*
...
#include "ui_ui_factory_rtc.h"
#include "ui_rotate_management.hpp"
#include "ui_tip_dialog.hpp"

#define TIMER_INTERVAL_COUNT 1000

ui_factory_rtc::ui_factory_rtc(QWidget *parent) :
    QtMainWindow(parent),
    ui(new Ui::ui_factory_rtc),
    test_end_(0),
    test_result_(0) {
...
/*
  if (rotate_signgleton->auto_test.auto_test_run)
                    ui->factory_rtc_back_btn->setEnabled(false);
*/

  connect(ui->factory_rtc_back_btn, SIGNAL(clicked()),
...
ui_factory_rtc::~ui_factory_rtc() {
  delete ui;
  delete pTimer;
}

void ui_factory_rtc::slot_for_factory_rtc_test() {
  ui->tip_label->
        setText(tr("\n\n\n  通过自动设置时间验证RTC测试是否正常工作"
                "正在设置中,请稍等..."));
  pTimer->start(TIMER_INTERVAL_COUNT);
...
void ui_factory_rtc::slot_for_call_function() {
...
void ui_factory_rtc::slot_for_back_btn() {
  if (test_end_ == 0) {
  ...
  ui_rotate_management *rotate_signgleton =
    ui_rotate_management::getInstance();
  rotate_signgleton->slot_for_factory_rtc(test_result_);
}

void ui_factory_rtc::proc_func() {
  test_result_ = ui_control_test_rtc();
  test_end_ = 1;
}

int ui_factory_rtc::is_test_end() {
  return test_end_;
}

void ui_factory_rtc::slot_for_exit() {
  // usleep(1000 * 1000);
  slot_for_back_btn();
}
