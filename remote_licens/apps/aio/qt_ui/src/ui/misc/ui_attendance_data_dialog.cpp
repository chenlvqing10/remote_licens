...
#include "ui_attendance_data_dialog.hpp"
#include "ui_ui_attendance_data_dialog.h"
#include "ui_rotate_management.hpp"

#define TRIGGER_INTERVAL 200

ui_attendance_data_dialog::ui_attendance_data_dialog(
  attendance_args_t *args, QWidget *parent) :QtDialog(parent),
  ui(new Ui::ui_attendance_data_dialog) {
  ...
  attendance_info = new attendance_args_t;
  attendance_info->id = args->id;
  attendance_info->cb = args->cb;
  ...
  m_group->addButton(ui->attendance_dialog_time_start, 0);
  m_group->addButton(ui->attendance_dialog_time_end, 1);
  ...
  counts[0].value = ui->attendance_dialog_unit_y;
  ...
  counts[1].value = ui->attendance_dialog_unit_m;
  ...
  counts[2].value = ui->attendance_dialog_unit_d;
  ...
  if (ptm->tm_year + 1900 != 1970) {
    ...
    .arg(str_mon_p)
    .arg(str_day);
  ui->attendance_dialog_time_start->setText(start);
  ui->attendance_dialog_time_end->setText(end);
  ui->attendance_dialog_time_start->setChecked(true);
...
void ui_attendance_data_dialog::slot_for_confirm() {
  long start = 0, end = 0;
  int ret = 0, ret1 = 0;
  if (attendance_info->cb) {
    QString s_txt = QString("%1 00:00:00")
      .arg(ui->attendance_dialog_time_start->text());
    QString e_txt = QString("%1 23:59:59")
      .arg(ui->attendance_dialog_time_end->text());
    ...
    attendance_info->cb->attendance_callback(
      attendance_info->id, QString("%1--%2").arg(s_txt).arg(e_txt));
  }
  deinit();
}

void ui_attendance_data_dialog::time_select_clicked(int id) {
...
void ui_attendance_data_dialog::slot_for_cancel() {
  deinit();
}

void ui_attendance_data_dialog::map_pressed(QWidget* w) {
  if (!w)
    return;
  for (int i = 0; i < TIME_UNITS; i++) {
    if (w == counts[i].add) {
...
void ui_attendance_data_dialog::slot_for_released() {
  pressed_index = -1;
  if (p_time->isActive()) {
    p_time->stop();
  }
...
void ui_attendance_data_dialog::slot_for_timeout() {
  if (-1 == pressed_index)
    return;
  count_unit_t *cur_unit = &counts[pressed_index];
...
void ui_attendance_data_dialog::deinit() {
  this->deleteLater();
}
