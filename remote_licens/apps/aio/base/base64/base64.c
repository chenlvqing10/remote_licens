 ...
 int base2file(char *base64data, char *file)
{
	...
	if (NULL == base64data || NULL == file) {
		ALOGE("base2file args is null\n");
		return -1;
	}
	bindata = malloc(strlen(base64data) * 3 / 4 + 1);
	if (NULL == bindata) {
		ALOGE("base2file: malloc failed\n");
		return -1;
	}
...
char *file2base(char *img_path)
{
	...
	if (img_path == NULL) {
		ALOGE("%s: %s is not exist", __func__, img_path);
		return NULL;
	}

	size = file_length(img_path);
	if (size <= 0) {
		ALOGE("%s: image path is invalid! skip!!!", __func__);
		return NULL;
	}

	bindata = malloc(size);
	if (bindata == NULL) {
		ALOGE("%s: bindata malloc failed", __func__);
		return NULL;
	}
	if (file_read(img_path, bindata, size) < 0) {
		ALOGE("%s: %s file_read  failed", __func__, img_path);
		SAFE_FREE(bindata);
		return NULL;
	}
	base64data = malloc(size * 4 / 3 + 4);
	if (base64data == NULL) {
		ALOGE("%s: base64data malloc failed", __func__);
		SAFE_FREE(bindata);
		return NULL;
	}
