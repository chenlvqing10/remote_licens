...
/**
 * Number of columns in each person registration
 * information when registering in bulk
 */
...
{
	check_ctx.is_proc_exit = is_exit;

	return 0;
}

/** Get the exit flag of recognitions
 * @is_proc_exit: a pointer of flag of exit
 *
 * return 0
 */
static int excel_reg_check_get_proc_exit_flag(int *is_proc_exit)
{
	*is_proc_exit = check_ctx.is_proc_exit;
 ...
 */
static int excel_reg_check_head_write(
		lxw_worksheet * p_write_ws, uint32_t row_number,
		xlsRow *p_row, WORD col_count)
{
	...
	worksheet_write_string(p_write_ws, row_number, fail_reason_col,
		clang_str("TEXT_REG_CHECK_RESULT_TITLE"), NULL);

	/* Show detail info title */
	fail_reason_col = col_count + 1;
	worksheet_write_string(p_write_ws, row_number, fail_reason_col,
		clang_str("TEXT_REG_CHECK_DETAIL_TITLE"), NULL);

	/* Show comparison results title */
	fail_reason_col = col_count + 2;
	worksheet_write_string(p_write_ws, row_number, fail_reason_col,
		clang_str("TEXT_REG_CHECK_COMPARE_RESULT_TITLE"), NULL);

	return 0;
}

/**
 * excel_reg_check_row_collect - Check the validity of each
 * column of data in each row of excel.
 * @p_row A pointer to 'xlsRow'
 * @col_count Number of columns per row
 ...
 */
static e_person_reg_code excel_reg_check_row_collect(
		xlsRow *p_row, WORD col_count,
		...
		char *dir_name)
{
	...
	assert((p_reg_check_info != NULL) &&
			(p_out_code != NULL) && (dir_name != NULL));
 ...
 * Returns 0 if success, otherwise not 0.
 */
static int excel_reg_check_row_write(
		lxw_worksheet * p_write_ws, uint32_t row_number,
...
static int excel_reg_check_file_sheet_proc(xlsWorkSheet *ws, char *dir_name)
{
	...
	/* Create an Excel file and write the information when there
	 * is a registration failure
	 */
	...
	excel_file_write_open(write_excel_file_name,
		clang_str("TEXT_EXCEL_REG_CHECK_RESULT_SHEET_NAME"),
	...
	for (int j = 0; j <= row_count; j++) {
		...
		ret = excel_reg_check_get_proc_exit_flag(&is_proc_exit);
		if ((ret == 0) && (1 == is_proc_exit)) {
			ALOGD("%s[%d] get_proc_exit_flag(%d) is_proc_exit = 1.",
				__func__, __LINE__, j);
			break;
		}
...
#if 0
	check_ctx.recg_param.threshold = 0.5;
	check_ctx.recg_param.jpeg_width_stride = 1280;
	check_ctx.recg_param.jpeg_height_stride = 720;
#else
	check_ctx.recg_param.threshold = recg_param.identify_threshold_score;
#endif
...
void *person_reg_check_proc(void *arg)
{
	...
	for (i = 0; i < (int)sheet_count; i++) {
		...
		excel_reg_check_file_sheet_proc(ws, dname);
	}
