...
H264VideoServerMediaSubsession::H264VideoServerMediaSubsession(UsageEnvironment & env,
	FramedSource * source, int channel) : OnDemandServerMediaSubsession(env, True)
...
H264VideoServerMediaSubsession * H264VideoServerMediaSubsession::createNew(
	UsageEnvironment & env, FramedSource * source, int channel)
{
	return new H264VideoServerMediaSubsession(env, source, channel);
}

FramedSource * H264VideoServerMediaSubsession::createNewStreamSource(
	unsigned clientSessionId, unsigned & estBitrate)
{
	return H264VideoStreamFramer::createNew(envir(), new H264VideoSource(envir(), channel_));
}
