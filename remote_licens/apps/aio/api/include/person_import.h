...
/**
 * When the registration fails, the failure information
...
typedef enum tag_person_import_error_type_e {
	...
	PERSON_IMPORT_USER_EXIT_ERR, /* During the import process,
					 * the user clicks the
...
/* check name is validate for use registerd */
int check_name_is_validate(const char *str);

/* check id is validate for use registerd */
