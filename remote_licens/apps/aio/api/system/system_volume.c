...
int system_hw_volume_set_value(int volume)
{
	...
	if (cout == 0) {
		ret = 0;
		tmp_volume = volume;
		ALOGI("%s: set volume tmp_volume : %d", __func__, tmp_volume);

		/* Set init volume 100(this will affect all) */
		lombo_set_master_volume(tmp_volume);
