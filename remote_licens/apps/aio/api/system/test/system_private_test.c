...
int main(int argc, const char **argv)
{
	...
	ret = system_get_private_data(SN_OFFSET, buf, 11);
	if (ret) {
		ALOGE("get SN failed");
		return -1;
	}
	...
	ret = system_get_private_data(SECRETNO_OFFSET, buf, 34);
	if (ret) {
		ALOGE("get SECRETNO failed");
		return -1;
	}
	ALOGD("SECRETNO:%s", buf);

	ret = system_get_private_data(DEVICEID_OFFSET, buf, 20);
	if (ret) {
		ALOGE("get DEVICEID failed");
		return -1;
	}
	ALOGD("DEVICEID:%s", buf);

	ret = system_get_private_data(QRCODE_OFFSET, buf, 101);
	if (ret) {
	...
	ret = system_set_private_data(RGBNIR_MAP_OFFSET,
			&w_map_params, sizeof(map_params_t));

	map_params_t r_map_params;
	memset(&r_map_params, 0, sizeof(map_params_t));
	ret = system_get_private_data(RGBNIR_MAP_OFFSET,
			&r_map_params, sizeof(map_params_t));
