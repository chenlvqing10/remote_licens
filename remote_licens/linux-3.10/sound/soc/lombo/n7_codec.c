/*
 * n7_codec.c - Common code for LomboTech SoCs
 *
...
/* write 1 to latch register, to update all the  data in AC registers */
static void latch(void __iomem *regbase)
{
	writel(1, regbase + N7_AC_ACL);
	udelay(1);
	writel(0, regbase + N7_AC_ACL);
}
...
static const struct snd_kcontrol_new n7ac_snd_controls[] = {
	SOC_DOUBLE("Input PGA Mute",		N7_AC_MCTL0, 2, 3, 1, 0),
	SOC_DOUBLE("Record Digital Mute",	N7_AC_MCTL0, 0, 1, 1, 0),
	SOC_SINGLE("Playback Digital Mute",	N7_AC_MCTL2, 0, 1, 0),
	SOC_SINGLE("Playback Analog Mute",	N7_AC_MCTL4, 2, 1, 0),

	SOC_DOUBLE_R_TLV("Record Master Volume", N7_AC_RECVL, N7_AC_RECVR,
		 0, 84, 0, record_tlv),
	SOC_DOUBLE_R_TLV("Input PGA Volume", N7_AC_IPGAVL, N7_AC_IPGAVR,
		 0, 19, 0, inputpga_tlv),
	SOC_SINGLE_TLV("Playback Master Volume", N7_AC_OPMVC,
		 0, 84, 0, playback_tlv),
};

/* Left Output Mixer */
static const struct snd_kcontrol_new left_mixer_controls[] = {
	SOC_DAPM_SINGLE("Left DAC OUT Switch",  N7_AC_OPMC, 0, 1, 0),
	SOC_DAPM_SINGLE("Left Loopout Switch",  N7_AC_OPMC, 1, 1, 0),
};
...
static const struct snd_soc_dapm_widget n7ac_dapm_widgets[] = {
	/* ADC DAPM WIDGETS */
	SND_SOC_DAPM_ADC("ADCL", "Capture", N7_AC_PWRIN, 0, 0),
	SND_SOC_DAPM_ADC("ADCR", "Capture", N7_AC_PWRIN, 1, 0),

	/* PGA DAPM WIDGETS */
	SND_SOC_DAPM_PGA("PGAL",  N7_AC_PWRIN, 2, 0, NULL, 0),
	SND_SOC_DAPM_PGA("PGAR",  N7_AC_PWRIN, 3, 0, NULL, 0),

	/* DAC DAPM WIDGETS */
	SND_SOC_DAPM_DAC("DACL", "Playback", N7_AC_PWROUT, 0, 0),
	SND_SOC_DAPM_DAC("DACR", "Playback", N7_AC_PWROUT, 0, 0),

	/* LINE OUT DRIVER */
	SND_SOC_DAPM_OUT_DRV("LINELDRV", N7_AC_PWROUT, 7, 0, NULL, 0),
	SND_SOC_DAPM_OUT_DRV("LINERDRV", N7_AC_PWROUT, 6, 0, NULL, 0),

	/* MICBIAS WIDGET */
	/* SND_SOC_DAPM_MICBIAS("MICBIAS", N7_AC_PWRM, 2, 0), */
...
static const struct snd_soc_dapm_route n7ac_dapm_routes[] = {
	...
	{"LINERDRV", NULL, "Left Output Mixer"},
...
static int n7ac_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	i2scfg = n7_ac_read(codec, N7_AC_I2SCFG);
	...
	n7_ac_write(codec, N7_AC_I2SCFG, i2scfg);
	...
	struct snd_soc_dai *dai)
{
	...
	n7_ac_write(codec, N7_AC_I2SDAFS, dacfs);
	n7_ac_write(codec, N7_AC_I2SADFS, adcfs);
...
static void n7_ac_poweron(struct snd_soc_codec *codec)
{
	u32 pwrm = n7_ac_read(codec, N7_AC_PWRM);

	pwrm |= 0x1;
	pwrm |= (0x1 << 2);
	n7_ac_write(codec, N7_AC_PWRM, pwrm);
}

static void n7_ac_poweroff(struct snd_soc_codec *codec)
{
	u32 pwrm = n7_ac_read(codec, N7_AC_PWRM);
				 ...
				 enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		if (codec->dapm.bias_level <= level)
			break;
		n7_ac_poweroff(codec);
		PRT_DBG("powr standby11\n");
		break;
...
{
	n7_ac_write(codec, N7_AC_HPM, 0x4);
	/* set default record master volume to 6db & 0db */
	n7_ac_write(codec, N7_AC_RECVL, 0x10);
	n7_ac_write(codec, N7_AC_RECVR, 0x14);
	/* set default record pga volume to 21db & 0db */
	n7_ac_write(codec, N7_AC_IPGAVL, 0x13);
	n7_ac_write(codec, N7_AC_IPGAVR, 0x4);
	return 0;
}
...
static int __init lombo_codec_init(void)
{
	int ret = 0;
	u32 p1 = 0, p2 = 0;
...
module_init(lombo_codec_init);

static void __exit lombo_codec_exit(void)
{
	u32 p1 = 0, p2 = 0;
