 ...
 *
 * ov2718 dvp sensor driver code for LomboTech
 *
...
static const cc_reg_t ov2718_hdr_reg_list[] = {
	{ 0x3000, 0x02 },
	{ 0x3001, 0x20 },
	{ 0x3002, 0x03 },
	{ 0x3003, 0x01 },
...
static int sensor_init(struct cc_sensor_info *cc_si)
{
	...
	ktime_get_ts(&ts_s);
	ret = cc_si->write_reg_list(cc_si, ov2718_hdr_reg_list,
	...
	for (i = 0; i < ARRAY_SIZE(ov2718_hdr_reg_list); i++) {
		if (ov2718_hdr_reg_list[i].reg == CC_REG_DELAY)
			continue;

		ret = cc_si->read_reg(cc_si, ov2718_hdr_reg_list[i].reg,
					&value);
		if (ret) {
			PRT_ERR("%s read i %d reg 0x%04x error\n",
				cc_si->name, i, ov2718_hdr_reg_list[i].reg);
			return ret;
		}

		if (ov2718_hdr_reg_list[i].val != value) {
			PRT_ERR("%s %d 0x%04x (0x%02x) != (0x%02x)\n",
				cc_si->name, i, ov2718_hdr_reg_list[i].reg,
				value, ov2718_hdr_reg_list[i].val);
		}
	}
	}
#endif
...
static const struct of_device_id ov2718_dvp_of_match[] = {
	{
		.compatible = DRIVER_NAME,
	},
	{ /* sentinel */ },
};
MODULE_DEVICE_TABLE(of, ov2718_dvp_of_match);

static struct platform_driver ov2718_dvp_driver = {
	.driver = {
		.of_match_table	= ov2718_dvp_of_match,
		.name		= DRIVER_NAME,
		.owner		= THIS_MODULE,
	},
	.probe = sensor_probe,
...
module_platform_driver(ov2718_dvp_driver);

MODULE_DESCRIPTION("OV2718-DVP sensor driver");
