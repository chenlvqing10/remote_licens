...
static const struct cc_frame_cfg frame_cfgs[] = {
	{
		...
		.mfreq = 1080000000,	/* mipi-csi clock */
	},
};
...
static int sensor_detect(struct cc_sensor_info *cc_si)
{
	...
	if (id != 0x2770) {
		PRT_ERR("ID wrong! (0x%04x != 0x2770)\n", id);
		return -ENODEV;
	}
...
static int sensor_set_stream(struct cc_sensor_info *cc_si, int on)
{
	...
	if (on) {
		ret = cc_si->write_reg(cc_si, 0x3012, 0x01);
		if (ret)
			PRT_ERR("%d\n", on);
	} else {
		ret = cc_si->write_reg(cc_si, 0x3012, 0x00);
		if (ret)
			PRT_ERR("%d\n", on);
	}
				...
				__u32 shutter_time)
{
	...
	ret = cc_si->write_reg(cc_si, 0x30b7, ((shutter_time >> 4) & 0xff));
	ret = cc_si->write_reg(cc_si, 0x30b6, ((shutter_time >> 12) & 0xff));
...
static int viss_isp_set_gain(struct cc_sensor_info *cc_si, float gain)
{
	...
	ret = cc_si->write_reg(cc_si, 0x30bb, (gain_multi << 2));

	ret = cc_si->write_reg(cc_si, 0x315a, ((final_gain) >> 8) & 0x3f);
	ret = cc_si->write_reg(cc_si, 0x315b, ((final_gain) & 0x00ff));

	ret = cc_si->write_reg(cc_si, 0x315c, ((final_gain) >> 8) & 0x3f);
	ret = cc_si->write_reg(cc_si, 0x315d, ((final_gain) & 0x00ff));

	ret = cc_si->write_reg(cc_si, 0x315e, ((final_gain) >> 8) & 0x3f);
	ret = cc_si->write_reg(cc_si, 0x315f, ((final_gain) & 0x00ff));
				...
				struct viss_isp_exp_gain *exp_gain)
{
	...
	ret = cc_si->write_reg(cc_si, 0x3467, 0x00);
	ret = cc_si->write_reg(cc_si, 0x3464, 0x04);
	...
	ret = cc_si->write_reg(cc_si, 0x3464, 0x14);
	ret = cc_si->write_reg(cc_si, 0x3467, 0x01);
...
static const struct of_device_id ov2718_mipi_of_match[] = {
	{
		.compatible = DRIVER_NAME,
	},
	{ /* sentinel */ },
};
MODULE_DEVICE_TABLE(of, ov2718_mipi_of_match);

static struct platform_driver ov2718_mipi_driver = {
	.driver = {
		.of_match_table	= ov2718_mipi_of_match,
		.name		= DRIVER_NAME,
		.owner		= THIS_MODULE,
	},
	.probe = sensor_probe,
...
module_platform_driver(ov2718_mipi_driver);

MODULE_DESCRIPTION("OV2718-MIPI sensor driver");
