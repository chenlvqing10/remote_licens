...
static int cmos_ar0230_init(ar0230_config_t *ar0230)
{
	...
	if (ar0230->hdr_mode)
		...
		ret = reg_list_write(client, LIST_AR0230_LINEAR_REGS);
	if (ret != 0)
		return ret;
...
static int viss_isp_sensor_hdr_mode(struct v4l2_subdev *sd,
				unsigned int *mode)
{
	ar0230_config_t *ar0230 = to_ar0230(sd);

	if (*mode != ar0230->hdr_mode) {
		ar0230->hdr_mode = *mode;
		ar0230_s_stream(sd, 0);
		ar0230_set_power(sd, 0);
		ar0230_set_power(sd, 1);
		ar0230_s_stream(sd, 1);
	}
	return 0;
}
...
static int viss_isp_set_shutter(struct v4l2_subdev *sd, __u32 shutter_time)
{
	...
	ret = reg_write16(client, 0x3012, (u16)shutter_time);
...
static int viss_isp_set_gain(struct v4l2_subdev *sd, __u32 gain)
{
	...
	if (ar0230->hdr_mode) {
		...
		if (a_gain < 16*16) {
			reg_write16(client, 0x3082, 0x08);
			ar0230->hdr_flag = 1;
		} else {
			reg_write16(client, 0x3082, 0x09);
			a_gain /= 16;
			ar0230->hdr_flag = 0;
		}
	}
...
static long ar0230_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_VISS_ISP_SENSOR_HDR_MODE:
		viss_isp_sensor_hdr_mode(sd, (unsigned int *)arg);
		break;
	default:
		return -EINVAL;
	}
...
static int ar0230_registered(struct v4l2_subdev *sd)
{
	...
	if (ar0230->skip_detect) {
		PRT_INFO("skip sensor ar0230 detect\n");
		ar0230->id = ID_AR0230;
		goto unlock;
	}
