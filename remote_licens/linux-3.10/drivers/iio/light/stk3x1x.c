...
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	50    //50
	#define STK_LT_N_CT	60			   //60
	#define STK_HT_N_CT	90			   //90
...
static int32_t stk3x3x_get_ir_reading(struct stk3x3x_data *ps_data, int32_t als_it_reduce);
#ifdef STK_TUNE0
...
#define ALS_CALI_SOURCE_LUX  1000

static int32_t stk3x3x_ps_calibration_step01(struct stk3x3x_data *ps_data){
...
static int32_t stk3x3x_ps_calibration_step02(struct stk3x3x_data *ps_data){
    //step01: cali near
    int32_t ret;
    uint8_t w_state_reg;    
    uint32_t reading, ps_sum = 0, counter = 0;   
    ...
    APS_INFO("%s: HT=%d, LT=%d\n", __func__, ps_data->ps_thd_h, ps_data->ps_thd_l);
    
    //need do write nvram
    write_nvram(ps_data->cali_near_increment); 
...
static int32_t stk3x3x_load_ps_calibration_config(struct stk3x3x_data *ps_data){

    //need do read nvram
    ps_data->cali_near_increment = read_nvram(); 

    //update ht, lt after read calibration
    ps_data->stk_ht_n_ct = ps_data->cali_near_increment;
    ps_data->stk_lt_n_ct = ps_data->stk_ht_n_ct - PS_FAR_DIFFER_VALUE;    
...
static int32_t stk3x3x_als_calibration(struct stk3x3x_data *ps_data){
    //cali ALS
    int32_t ret = 0;
    uint8_t w_state_reg;    
    uint32_t reading, als_sum = 0, counter = 0;

    if(ps_data->als_enabled == false){
        //enable als
        ret = stk3x3x_get_state(ps_data);
        if(ret < 0)
            return ret;
    ...
    ps_data->cali_als = (als_sum / ALS_CALI_COUNT);
    if(ps_data->cali_als == 0){
        APS_INFO("%s: cali als = 0\n", __func__);
        return 0xFFFF;
    }
    ps_data->lux_factor = (ALS_CALI_SOURCE_LUX * 1000) / ps_data->cali_als;
    APS_INFO("%s: cali factor = %d\n", __func__, ps_data->lux_factor);
    //need do write nvram
    write_nvram(ps_data->lux_factor); 
...
static int32_t stk3x3x_load_als_calibration_config(struct stk3x3x_data *ps_data){

    //need do read nvram
    ps_data->lux_factor = read_nvram(); 
}
#endif

static void dump_data_reg(struct stk3x3x_data *ps_data)
{
	int ret, i;
	uint8_t thdh1_ps, thdh2_ps, thdl1_ps, thdl2_ps;
	uint8_t thdh1_als, thdh2_als, thdl1_als, thdl2_als;
	uint8_t data1_ps, data2_ps;
	uint8_t data1_als, data2_als;

	u16 thdh_ps, thdl_ps;
	u16 thdh_als, thdl_als;
	u16 data_ps, data_als;

	uint8_t* values[12] = {&thdh1_ps, &thdh2_ps, &thdl1_ps, &thdl2_ps,
				&thdh1_als, &thdh2_als, &thdl1_als, &thdl2_als,
				&data1_ps, &data2_ps, &data1_als, &data2_als};
	unsigned char reg[12] = { 0x06, 0x07, 0x08, 0x09,
				0x0A, 0x0B, 0x0C, 0x0D,
				0x11, 0x12, 0x13, 0x14};
	uint8_t* tmp;

	for (i = 0; i < 12; i++) {
		ret = stk3x3x_i2c_smbus_read_byte_data(ps_data->client, reg[i]);
		if (ret < 0) {
			printk(KERN_ERR "%s:dump_data_reg fail, ret=%d\n", __func__, ret);
			return;
		}
		tmp = values[i];
		*tmp = (uint8_t)ret;
	...
	printk(KERN_INFO "============= dump_data_reg =============\n");

	thdh_ps = (thdh1_ps << 8) | thdh2_ps;
	printk(KERN_INFO "thdh_ps = 0x%4x\n", thdh_ps);

	thdl_ps = (thdl1_ps << 8) | thdl2_ps;
	printk(KERN_INFO "thdl_ps = 0x%4x\n", thdl_ps);

	thdh_als = (thdh1_als << 8) | thdh2_als;
	printk(KERN_INFO "thdh_als = 0x%4x\n", thdh_als);

	thdl_als = (thdl1_als << 8) | thdl2_als;
	printk(KERN_INFO "thdl_als = 0x%4x\n", thdl_als);

	data_ps = (data1_ps << 8) | data2_ps;
	printk(KERN_INFO "data_ps = 0x%4x\n", data_ps);

	data_als = (data1_als << 8) | data2_als;
	printk(KERN_INFO "data_als = 0x%4x\n", data_als);
}

static void stk3x3x_dump_reg(struct stk3x3x_data *ps_data)
{	
	int ret;
	unsigned char reg[7]={0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x10 /*, 0x4F, 0xA0, 0xA1, 0xDB, 0xA4, 0x3E, 0x3F*/};
	uint8_t cnt;
	for(cnt = 0; cnt < 7; cnt++){
		...
		printk(KERN_INFO "%s: reg[0x%2X]=0x%2X\n", __func__, reg[cnt], ret);
	}
}
...
#ifdef STK_ALSPS_CALIBRATION
    uint32_t lux;
    lux = (alscode * ps_data->lux_factor) / 1000;
    return lux;
#else
...
#ifdef STK_ALSPS_CALIBRATION
    uint32_t alscode;   
    alscode = (lux * 1000) / ps_data->lux_factor;
...
//#endif	
	...
	// config interrupt directly, enable INT_PS and INT_ALS
	//ps_data->int_reg = 0x0c;
	pr_err("########### ps_data->int_reg = 0x%x ##########\n", ps_data->int_reg);
	return;
}
...
static int32_t stk3x3x_init_all_reg(struct stk3x3x_data *ps_data)
{
	...
	printk(KERN_INFO "%s: i2c write to reg[STK_INT_REG]: 0x%2x\n", __func__, ps_data->int_reg);
...
#ifdef STK_GES		
static int32_t stk3x3x_set_gsctrl(struct stk3x3x_data *ps_data, uint8_t state)
{
	int ret;		
    ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client,STK_GSCTRL_REG, state);	
	if(ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
}

static int32_t stk3x3x_get_gsctrl(struct stk3x3x_data *ps_data)
{	
	int ret;
    ret = stk3x3x_i2c_smbus_read_byte_data(ps_data->client,STK_GSCTRL_REG);	
	if(ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
}
...
#if 0
static uint32_t stk3x3x_get_ges_reading(struct stk3x3x_data *ps_data, unsigned int *ges0,
				unsigned int *ges1, unsigned int *ges2)
{
	...
	while(stk_ges_op[no].ops[1] != 0)
	{
		...
		if(stk_ges_op[no].action.rw_len_retry & 0x80)
		{
			while(retry != 0)
			{
				err = stk3x3x_i2c_read_data(ps_data->client, 
		...
		else
		{
			while(retry != 0)
			{
				err = stk3x3x_i2c_write_data(ps_data->client, stk_ges_op[no].action.reg, 
								len, &reg_val_crit);		
				if(err < 0)
				{
...
#else
static uint32_t stk3x3x_get_ges_reading(struct stk3x3x_data *ps_data, unsigned int *ges0,unsigned int *ges1,unsigned int *ges2)
{
	unsigned char value[4];
	int err, retry = 10;
	
	do {
		err = stk3x3x_get_flag(ps_data);	
		if(err < 0)
			return err;
	...
	err = stk3x3x_i2c_read_data(ps_data->client, 0x24, 4, &value[0]);
	if(err < 0)
	{
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, err);
		return err;
...
static int32_t stk3x3x_enable_ges(struct stk3x3x_data *ps_data, uint8_t enable, uint8_t mode)
{
    int32_t ret;
	...
	if(enable)
	{
		...
		ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client, STK_WAIT_REG, 0);
		if (ret < 0)
		{
			printk(KERN_ERR "%s: write i2c error\n", __func__);
			return ret;
		...
		if(mode == 2)
		{
			ret = stk3x3x_get_gsctrl(ps_data);
			if(ret < 0)
				return ret;		
			gsctrl_reg = ret & 0xF3;
#ifdef STK_POLL_PS
			...
			gsctrl_reg |= 0x0C;
#endif			
			ret = stk3x3x_set_gsctrl(ps_data, gsctrl_reg);
			if(ret < 0)
				return ret;			
#ifdef STK_POLL_PS
	...
	else
	{
		...
		ret = stk3x3x_set_state(ps_data, 0);
		if(ret < 0)
			return ret;		
		ps_data->state_reg = 0;
...
static int32_t stk3x3x_enable_ps(struct stk3x3x_data *ps_data, uint8_t enable, uint8_t validate_reg)
{
	...
	if(ps_data->ges_enabled && enable)
	{
		...
		stk3x3x_enable_ges(ps_data, 0, 1);
    ...
    if(enable)
	{
		...
		// }
		stk3x3x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
		stk3x3x_set_ps_thd_l(ps_data, 0);		
#else
#ifdef CALI_PS_EVERY_TIME		
		...
		printk(KERN_INFO "%s: boot cali=%d, H=%d, L=%d\n", __func__, ps_data->boot_cali, ps_data->ps_high_thd_boot, ps_data->ps_low_thd_boot);
	...
	else
	{
		...
		if(ps_data->re_enable_ges)
		{
			...
			stk3x3x_enable_ges(ps_data, 1, 1);
...
static int32_t stk3x3x_get_als_reading(struct stk3x3x_data *ps_data)
{
	...
	if(ps_data->p_wv_r_bd_with_co & 0b010)
	{
		...
		{
			ir_data = stk3x3x_get_ir_reading(ps_data, STK_ALS_READ_IRS_IT_REDUCE);
#ifdef STK_IRS				
			if(ir_data > 0)
...
static int stk_als_ir_skip_als(struct stk3x3x_data *ps_data)
{
	int ret;
	unsigned char value[2];	
...
static void stk_als_ir_get_corr(struct stk3x3x_data *ps_data, int32_t als)
{
...
static int stk_als_ir_run(struct stk3x3x_data *ps_data)
{
	int ret;
...
static int32_t stk3x3x_set_irs_it_slp(struct stk3x3x_data *ps_data, uint16_t *slp_time, int32_t ials_it_reduce)
{
	uint8_t irs_alsctrl;
	int32_t ret;
	...
	ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client, STK_ALSCTRL_REG, irs_alsctrl);
	if (ret < 0)
	{
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;		
...
static int32_t stk3x3x_get_ir_reading(struct stk3x3x_data *ps_data, int32_t als_it_reduce)
{
	...
	ret = stk3x3x_set_irs_it_slp(ps_data, &irs_slp_time, als_it_reduce);
	if(ret < 0)
		goto irs_err_i2c_rw;
	...
	ret = stk3x3x_set_state(ps_data, w_reg);
	if(ret < 0)
		goto irs_err_i2c_rw;
	msleep(irs_slp_time);	
	...
	ret = stk3x3x_set_flag(ps_data, ret, STK_FLG_IR_RDY_MASK);
    if (ret < 0)
		goto irs_err_i2c_rw;
	
	ret = stk3x3x_i2c_read_data(ps_data->client, STK_DATA1_IR_REG, 2, &value[0]);
	if(ret < 0)
	{
		printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
		goto irs_err_i2c_rw;
...
static int stk3x3x_validate_n_handle(struct i2c_client *client) 
{
	...
	struct stk3x3x_data *ps_data = iio_priv(indio_dev);
	int err;
...
static ssize_t stk_als_ir_code_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x3x_data *ps_data =  dev_get_drvdata(dev);		
    int32_t reading;
    reading = stk3x3x_get_ir_reading(ps_data, STK_IRS_IT_REDUCE);
    return scnprintf(buf, PAGE_SIZE, "%d\n", reading);	
}
...
static ssize_t stk_ges_code_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ret = stk3x3x_get_ges_reading(ps_data, &gest0, &gest1, &gest2);
	if(ret < 0)
		return ret;
	else if(ret == 0xFFFF)
...
static ssize_t stk_ges_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case 0:
		...
		if(ps_data->ges_enabled == 1)
			...
			stk3x3x_enable_ges(ps_data, 0, 2);	
		mutex_unlock(&ps_data->io_lock);		
		break;
	case 1:
		mutex_lock(&ps_data->io_lock);
	...
	case 2:
		mutex_lock(&ps_data->io_lock);
		stk3x3x_enable_ges(ps_data, 1, 2);
		mutex_unlock(&ps_data->io_lock);		
		break;
	default:
		printk(KERN_ERR "%s, invalid value %d\n", __func__, *buf);
...
static int stk_tune1_ct_det(struct stk3x3x_data *ps_data, uint32_t ps)
{
	int ct = 0;
	int err;
...
static int stk_tune1_fin_det(struct stk3x3x_data *ps_data, uint32_t ps, int32_t nf_flag)
{
	...
	if(nf_flag == 0)
	{
		if(ps >= STK_FIN_THD)
		{
			...
			ps_data->ps_thd_l = STK_PS_HIGHER_THDL;
#ifdef STK_POLL_PS
			stk3x3x_set_ps_thd_h(ps_data, STK_PS_HIGHER_THDH);
			stk3x3x_set_ps_thd_l(ps_data, STK_PS_HIGHER_THDL);		
#endif
...
static int stk_tune1_ps_int_handle(struct stk3x3x_data *ps_data, uint32_t ps_reading)
{
	int ret;
	/*int cnt;*/
	int32_t nf_state;
	...
	case STK_PSINT_NORM:
		...
		{
			stk3x3x_set_ps_thd_h(ps_data, STK_FIN_THD);
			stk3x3x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
	...
	if(ret == 1)
	{
		...
		{
			ps_data->stk_int_handle_state = STK_PSINT_NORM;
			stk3x3x_set_ps_thd_h(ps_data, 0xffff);
			stk3x3x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
	...
	{
		stk3x3x_set_ps_thd_h(ps_data, 0);
		stk3x3x_set_ps_thd_l(ps_data, 0xffff);
...
#if ((STK_INT_PS_MODE == 0x03) || (STK_INT_PS_MODE	== 0x02))	
	printk(KERN_INFO "%d: request_any_context_irq\n", __LINE__);
...
{	
	...
	/*
	pdata->rst_pin = of_get_named_gpio_flags(np, "stk,rst-gpio",
				0, &pdata->rst_flags);
	if (pdata->rst_pin < 0) {
		dev_err(dev, "Unable to read rst-gpio\n");
		return pdata->rst_pin;
	}
	
	rc = gpio_request_one(pdata->rst_pin, GPIOF_OUT_INIT_HIGH, "stk-rst");
        if(rc < 0)
        {
                printk(KERN_ERR "%s: gpio_request, err=%d", __func__, rc);
                return rc;
        }
	msleep(10);
...
int log_reg_thread(void *data)
{
	struct stk3x3x_data *ps_data = (struct stk3x3x_data *)data;
	while (1) {
...
int create_log_reg_thread(void *data)
{
	/*int err;*/
	
	printk(KERN_INFO "%d: create a thread to log register info\n", __LINE__);
	test_task = kthread_create(log_reg_thread, data, "test_task");
	if (IS_ERR(test_task)) {
		printk(KERN_ERR "%d: kthread_create error\n", __LINE__);
		return -1;
	}
...
static int stk3x1x_read_chan(struct stk3x3x_data *ps_data,
				struct iio_chan_spec const *chan)
{
	...
	if (chan->type == IIO_LIGHT) {
		pr_info("##### IIO_LIGHT #####\n");
		reg = STK_DATA1_ALS_REG;
	} else if (chan->type == IIO_PROXIMITY) {
		pr_info("##### IIO_PROXIMITY #####\n");
		reg = STK_DATA1_PS_REG;
	} else {
		pr_warn("unknown chan type: %d\n", chan->type);
		return -1;
	}

	ret = stk3x3x_i2c_read_data(ps_data->client, reg, 2, &value[0]);
	if (ret < 0) {
		printk(KERN_ERR "%s fail, ret=0x%x\n", __func__, ret);
		return ret;
	}

	val = (int)((value[0] << 8) | value[1]);	
	return val;
}


static int stk3x1x_read_raw(struct iio_dev *indio_dev,
			      struct iio_chan_spec const *chan,
			      int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		*val = stk3x1x_read_chan(ps_data, chan);
		if (*val < 0)
			return *val;
...
/* Demo */
static const struct iio_chan_spec stk_als_channels[] = {
	STK3x1x_CHANNEL(IIO_LIGHT),
	STK3x1x_CHANNEL(IIO_PROXIMITY),
};

static const struct iio_info stk_iio_info = {
	.read_raw = stk3x1x_read_raw,
};
                        ...
                        const struct i2c_device_id *id)
{
	...
	indio_dev = iio_device_alloc(sizeof(*ps_data));
	if (indio_dev == NULL) {
		printk(KERN_ERR "%s: iio_device_alloc error\n", __func__);
		return -ENOMEM;
	}
	ps_data = iio_priv(indio_dev);
	i2c_set_clientdata(client, indio_dev);

	indio_dev->name = dev_name((const struct device *)DEVICE_NAME);
	indio_dev->info = &stk_iio_info;
	indio_dev->modes = INDIO_DIRECT_MODE;
	indio_dev->channels = stk_als_channels;
	indio_dev->num_channels = ARRAY_SIZE(stk_als_channels);

	err = iio_device_register(indio_dev);
	if (err) {
		printk(KERN_ERR "%s: iio_device_register error, ret = %d\n", __func__, err);
...
#ifdef STK_ALSPS_CALIBRATION
    stk3x3x_load_ps_calibration_config(ps_data);   
    
    ps_data->lux_factor = 1000;    
    stk3x3x_load_als_calibration_config(ps_data);
#endif
...
//#ifdef CONFIG_HAS_EARLYSUSPEND
	...
	ps_data->stk_early_suspend.suspend = stk3x3x_early_suspend;
	ps_data->stk_early_suspend.resume = stk3x3x_late_resume;
