...
static void lombo_uart_rs485_gpio_set_output(struct uart_port *port,
							 unsigned int val);
...
static void lombo_uart_stop_tx(struct uart_port *port)
{
	...
	if (eup->rs485_enable == 1) {
		while (!(csp_uart_tx_fifo_empty(port->membase)))
			;

		if (eup->rs485_gpio_ctrl == 0) {
			if (eup->rs485_sw_enable == 1) {
				csp_uart_rs485_de_enable(port->membase, 0);
				csp_uart_rs485_re_enable(port->membase, 1);
			}
		} else {
			udelay(162);
			lombo_uart_rs485_gpio_set_output(port, 0);
		}
	}
...
static void lombo_uart_start_tx(struct uart_port *port)
{
	...
	if (eup->rs485_enable == 1) {
		if (eup->rs485_gpio_ctrl == 0) {
			if (eup->rs485_sw_enable == 1) {
				csp_uart_rs485_de_enable(port->membase, 1);
				csp_uart_rs485_re_enable(port->membase, 0);
			}
		} else {
			lombo_uart_rs485_gpio_set_output(port, 1);
		}
	}
...
static int lombo_uart_startup(struct uart_port *port)
{
	...
	if (eup->rs485_enable == 1) {
		...
		if (eup->rs485_gpio_ctrl == 0) {
			csp_uart_rs485_de_pol_enable(port->membase, 1);
			csp_uart_rs485_re_pol_enable(port->membase, 0);

			if (eup->rs485_sw_enable == 1) {
				csp_uart_rs485_mode_config(port->membase,
							SW_HALF_DP_MODE);
				csp_uart_rs485_de_enable(port->membase, 0);
				csp_uart_rs485_re_enable(port->membase, 1);
			} else {
			...
			csp_uart_rs485_enable(port->membase, 1);
							...
							unsigned int val)
{
	...
	gpio_set_value(eup->rs485_pin, val);
}
...
static int __init lombo_console_setup(struct console *co, char *options)
{
	...
	if (eup->rs485_enable == 1) {
		...
		csp_uart_rs485_de_pol_enable(port->membase, 1);
		csp_uart_rs485_re_pol_enable(port->membase, 0);
		csp_uart_rs485_mode_config(port->membase, HW_HALF_DP_MODE);
		csp_uart_rs485_de_enable(port->membase, 1);
		csp_uart_rs485_re_enable(port->membase, 1);
		csp_uart_rs485_enable(port->membase, 1);
	}
...
static int lombo_uart_probe(struct platform_device *pdev)
{
	...
	err = of_property_read_u32(np, "uart_rs485_enable", &eup->rs485_enable);
	if (err)
		eup->rs485_enable = 0;

	err = of_property_read_u32(np, "uart_rs485_sw_enable",
							&eup->rs485_sw_enable);
	if (err)
		eup->rs485_sw_enable = 0;

	err = of_property_read_u32(np, "uart_rs485_gpio_ctrl",
							&eup->rs485_gpio_ctrl);
	if (err)
		eup->rs485_gpio_ctrl = 0;

	if (eup->rs485_enable == 1 && eup->rs485_gpio_ctrl == 1) {
		/*
		 * 1. get gpio number info from dts
		 * property name out must be consistent with dts
		 */
		eup->rs485_pin = of_get_named_gpio(np, "rs485_gpio", 0);
		if (eup->rs485_pin < 0) {
			PRT_ERR("uart%d rs485_gpio property not found\n",
					pdev->id);
			return -EINVAL;
		}

		/* 2. check gpio is valid */
		if (!gpio_is_valid(eup->rs485_pin)) {
			PRT_ERR("uart%d rs485gpio %d invalid\n",
						pdev->id, eup->rs485_pin);
			return -EINVAL;
		}

		/* 3. request gpio */
		err = devm_gpio_request(&pdev->dev, eup->rs485_pin,
							"rs485_gpio");
		if (err < 0) {
			PRT_ERR("uart%d request rs485_gpio%d fail\n",
						pdev->id, eup->rs485_pin);
			return err;
		}

		/*
		 * 4. set direction and output value
		 * set output function and output high level
		 */
		err = gpio_direction_output(eup->rs485_pin, 1);
		if (err < 0) {
			PRT_ERR("uart%d set rs485_gpio%d input fail\n",
						pdev->id, eup->rs485_pin);
			return err;
		}
	}
...
static int lombo_uart_remove(struct platform_device *pdev)
{
	...
	if (eup->rs485_enable == 1 && eup->rs485_gpio_ctrl == 1)
		devm_gpio_free(&pdev->dev, eup->rs485_pin);
...
static struct clk *g_clk_parent;
static unsigned long g_uart_rate;
...
static int lombo_uart_suspend(struct device *dev)
{
	...
	g_uart_rate = clk_get_rate(eup->uart_clk);

	g_clk_null = clk_get(NULL, "null_clk");
	g_clk_parent = clk_get_parent(eup->uart_clk);
	clk_set_parent(eup->uart_clk, g_clk_null);

	/* disable uart clk */
	clk_disable_unprepare(eup->apb_gate);
	clk_disable_unprepare(eup->apb_reset);
...
static int lombo_uart_resume(struct device *dev)
{
	...
	clk_prepare_enable(eup->apb_gate);
	clk_prepare_enable(eup->apb_reset);

	/* reset the parent */
	clk_set_parent(eup->uart_clk, g_clk_parent);
	clk_set_rate(eup->uart_clk, g_uart_rate);
	clk_put(g_clk_null);
