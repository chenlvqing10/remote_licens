...
{
	if (pb->bl_gpio)
		gpio_direction_output(pb->bl_gpio,
				ACTIVE(pb->bl_gpio_flag));
}
				  ...
				  struct platform_pwm_backlight_data *data)
{
	...
	if (!prop) {
		data->max_brightness = BRIGHTNESS_MAX_DEF;
		data->dft_brightness = BRIGHTNESS_DEF;
		return 0;
	}
...
static int pwm_backlight_probe(struct platform_device *pdev)
{
	...
	pb->bl_gpio = of_get_named_gpio_flags(np, "bl_gpio", 0, &flags);
	pb->bl_gpio_flag = flags;
	if (!gpio_is_valid(pb->bl_gpio)) {
		pr_err("bl gpio %d invalid\n", pb->bl_gpio);
		pb->bl_gpio = 0;
	} else {
		ret = devm_gpio_request(&pdev->dev, pb->bl_gpio, "bl_gpio");
		if (ret < 0) {
