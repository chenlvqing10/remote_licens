...
#if defined(CONFIG_DRM_LOMBO) && defined(CONFIG_DRM_FBDEV_EMULATION)

#include <drm/lombo_drm.h>

extern int lombo_fb_set_transparent_rect(struct fb_info *info,
	struct drm_lombo_rect_wh *rect);
extern int lombo_fb_get_transparent_rect(struct fb_info *info,
	struct drm_lombo_rect_wh *rect);

static ssize_t store_transparent_rect(struct device *device,
			 struct device_attribute *attr,
			 const char *buf, size_t count)
{
	struct fb_info *fb_info = dev_get_drvdata(device);
	...
	rect.x = simple_strtoul(buf, &last, 0);

	last++;
	if (last - buf >= count)
		return -EINVAL;
	rect.y = simple_strtoul(last, &last, 0);

	last++;
	if (last - buf >= count)
		return -EINVAL;
	rect.w = simple_strtoul(last, &last, 0);

	last++;
	if (last - buf >= count)
		return -EINVAL;
	rect.h = simple_strtoul(last, &last, 0);

	err = lombo_fb_set_transparent_rect(fb_info, &rect);
	if (err < 0)
		return err;
...
static ssize_t show_transparent_rect(struct device *device,
			struct device_attribute *attr, char *buf)
{
	struct fb_info *fb_info = dev_get_drvdata(device);
	struct drm_lombo_rect_wh rect = { 0 };

	if (lombo_fb_get_transparent_rect(fb_info, &rect))
		return 0;
...
#endif
#if defined(CONFIG_DRM_LOMBO) && defined(CONFIG_DRM_FBDEV_EMULATION)
	__ATTR(transparent_rect, S_IRUGO | S_IWUGO,
		show_transparent_rect, store_transparent_rect),
#endif
};
