...
extern void lombo_lfeosc_fanout_enable(u32 enable);
...
#ifndef BOARD_SLEEP_CLOCK
		lombo_lfeosc_fanout_enable(1);
#endif
...
int dhd_wlan_init_gpio(void)
{
	...
	char *wlan_node = "lombo,wireless-pm";
	...
	*/
	...
	if (!root_node) {
		printf("failed to get device node of wireless_pm\n");
		return -ENODEV;
	}

	gpio_wl_host_wake = of_get_named_gpio(root_node, "wifi_wakeup_host", 0);
	if (gpio_wl_host_wake < 0) {
		printf("failed to get named gpio\n");
		return -ENODEV;
	}
	...
	if (gpio_wl_host_wake >= 0) {
		err = gpio_request(gpio_wl_host_wake, "wifi_wakeup_host");
		if (err < 0) {
