			 ...
			 u8 *key_value)
{
	...
	*key_value = point_data[3 + 8 * touch_num];
	...
	for (i = 0; i < touch_num; i++) {
		...
		/* transformation for p2 and p2s */
		if (ts->pdata->transform_type == 1) {
			int old_x = points[i].x;
			int old_y = points[i].y;
			points[i].x = ts->pdata->abs_size_y - old_y;
			points[i].y = old_x;
		}

		dev_dbg(&ts->client->dev, "[%d][%d %d %d]\n",
			points[i].id, points[i].x, points[i].y, points[i].p);
		pr_info("EVENT:[%d][%d %d %d]\n",
			points[i].id, points[i].x, points[i].y, points[i].p);
...
static int gtp_find_valid_cfg_data(struct goodix_ts_data *ts)
{
	...
	dev_info(&ts->client->dev, "Read Config data from dts, length: %d\n",
		 cfg->length);
#else
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	if (SUCCESS == ret) {
		dev_info(&ts->client->dev, "check firmware opr_buf = 0x%02x 0x%02x\n", opr_buf[0], opr_buf[1]);
		if (opr_buf[0] != 0xBE) {
...
static int gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	if (gpio_is_valid(ts->pdata->irq_gpio)) {
		...
		gpio_direction_input(ts->pdata->irq_gpio);
		dev_info(&ts->client->dev, "Success request irq-gpio = %d\n", ts->pdata->irq_gpio);
	}
	...
	if (gpio_is_valid(ts->pdata->rst_gpio)) {
		...
		gpio_direction_input(ts->pdata->rst_gpio);
		dev_info(&ts->client->dev,  "Success request rst-gpio = %d\n", ts->pdata->rst_gpio);
	}
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	pdata->transform_type = 0;
	of_property_read_u32(np, "goodix,transform-type",
			     &pdata->transform_type);
	if (pdata->transform_type)
		dev_info(dev, "transform-type = %d\n", pdata->transform_type);
...
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 sid)
{
	...
	char cfg_name[18];
	int ret, cnt, i;
	u32 temp_cfg[GTP_CONFIG_MAX_LENGTH];

	snprintf(cfg_name, sizeof(cfg_name), "goodix,cfg-group%d", sid);

	cnt = of_property_count_u32_elems(np, cfg_name);
	if (cnt < 0) {
		pr_err("of_property_count_u32_elems err: %d\n", cnt);
		*cfg_len = 0;
		ret = -EPERM;
	}

	ret = of_property_read_u32_array(np, cfg_name, temp_cfg, cnt);
	if (ret) {
