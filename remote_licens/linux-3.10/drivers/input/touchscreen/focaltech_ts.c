...
*  Return:
*******************************************************************************/
int fts_i2c_read(struct i2c_client *client,
...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	for (i = 0; i < FTS_MAX_POINTS; i++) {
		...
		event->au16_x[i] =
		    (s16) (buf[FTS_TOUCH_X_H_POS +
		    FTS_ONE_TCH_LEN * i] & 0x0F) <<
		    8 | (s16) buf[FTS_TOUCH_X_L_POS +
		    FTS_ONE_TCH_LEN * i];
		event->au16_y[i] =
		    (s16) (buf[FTS_TOUCH_Y_H_POS +
			FTS_ONE_TCH_LEN * i] & 0x0F) <<
		    8 | (s16) buf[FTS_TOUCH_Y_L_POS +
		    FTS_ONE_TCH_LEN * i];
	...
	for (i = 0; i < event->touch_point; i++) {
		...
		if (event->au8_touch_event[i] == FTS_TOUCH_DOWN ||
			...
			input_report_abs(data->input_dev,
					ABS_MT_PRESSURE,
					event->pressure[i]);
		#if FOR_CALIBRATE_DEBUG
			pr_info("report ABS_PRESSURE for tslib\n");
			input_report_abs(data->input_dev,
					ABS_PRESSURE,
					event->pressure[i]);
		#endif
			...
			pr_info("focaltech tp report event -> x: %d, y: %d\n",
				event->au16_x[i], event->au16_y[i]);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	rc = of_property_read_u32(np,
				"focaltech,fw-delay-era-flsh-ms",
				&temp_val);
	if (rc && (rc != -EINVAL))
