...
{	{ DRM_MODE_CONNECTOR_Unknown, "Unknown", 0 },
	...
	{ DRM_MODE_CONNECTOR_DPI, "DPI", 0},
	{ DRM_MODE_CONNECTOR_WRITEBACK, "Writeback", 0},
};
			...
			struct drm_file *file_priv)
{
	...
	for (i = 0; i < plane->format_count; i++)
		if (fb->format->format == plane->format_types[i])
			break;
...
static int framebuffer_check(const struct drm_mode_fb_cmd2 *r)
{
	...
	for (i = 0; i < num_planes; i++) {
		...
		int has_bo = (r->modifier[i] ==
			DRM_FORMAT_MOD_LOMBO_SC) ? 0 : 1;

		if (r->modifier[i] == DRM_FORMAT_MOD_LOMBO_FB_ARRAY)
			has_bo = 0;

		if (has_bo && !r->handles[i]) {
		...
		if (has_bo && (r->pitches[i] < width * cpp)) {
