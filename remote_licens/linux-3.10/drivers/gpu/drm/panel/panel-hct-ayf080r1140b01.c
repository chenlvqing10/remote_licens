...
#define AYF_GPIO_ACTIVE(flag) (((flag) & OF_GPIO_ACTIVE_LOW) ? 0 : 1)
#define AYF_GPIO_INACTIVE(flag) (((flag) & OF_GPIO_ACTIVE_LOW) ? 1 : 0)

#define AYF_WIDTH	(800)
#define AYF_HEIGHT	(1280)

/* timing for tcon host */
#define AYF_HBP		(64)
#define AYF_HFP		(64)
#define AYF_HSW		(28)
#define AYF_HACT	(AYF_WIDTH)
#define AYF_VBP		(4)
#define AYF_VFP		(6)
#define AYF_VSW		(4)
#define AYF_VACT	(AYF_HEIGHT)
#define AYF_VT		(AYF_VBP + AYF_VFP + AYF_VSW + AYF_VACT)
#define AYF_DCLK	(74250000)
...
struct panel_ayf080r1140b01_desc {
...
struct panel_ayf080r1140b01 {
	struct drm_panel base;
	bool prepared;
	bool enabled;

	struct device *dev;
	const struct panel_ayf080r1140b01_desc *desc;
...
static inline struct panel_ayf080r1140b01 *to_panel_ayf080r1140b01(
	struct drm_panel *panel)
{
	return container_of(panel, struct panel_ayf080r1140b01, base);
}

static int panel_ayf080r1140b01_disable(struct drm_panel *panel)
{
	struct panel_ayf080r1140b01 *p = to_panel_ayf080r1140b01(panel);
	...
	if (p->bl_gpio)
		gpio_direction_output(p->bl_gpio,
			AYF_GPIO_INACTIVE(p->bl_gpio_flag));
...
static int panel_ayf080r1140b01_unprepare(struct drm_panel *panel)
{
	struct panel_ayf080r1140b01 *p = to_panel_ayf080r1140b01(panel);
	...
	if (p->enable_gpio)
		gpio_direction_output(p->enable_gpio,
			AYF_GPIO_INACTIVE(p->enable_gpio_flag));
...
static int ayf080r1140b01_mipi_dsi_dcs_write_array(
...
static u8 ayf080r1140b01_init_code[] = {
	/* Page0 */
	2, 0xE0, 0x00,
	...
	/* setting DSI option for read back */
	2, 0x80, 0x03,
	...
	/* Set VCOM */
	2, 0x00, 0x00,
	2, 0x01, 0xA0,	/* VCOM=-1.44V */

	/* Set Gamma Power, VGMP,VGMN,VGSP,VGSN */
	2, 0x17, 0x00,
	2, 0x18, 0xB1,	/* VGMP=4.325 */
	2, 0x19, 0x01,	/* VGSP=0.3V */
	2, 0x1A, 0x00,
	2, 0x1B, 0xB1,	/* VGMN=4.325V */
	2, 0x1C, 0x01,	/* VGSN=-0.3V */

	/* Set Gate Power */
	2, 0x1F, 0x3E,	/* VGH_R  = 15V */
	2, 0x20, 0x28,	/* VGL_R  = -11V */
	2, 0x21, 0x28,	/* VGL_R2 = -11V */
	2, 0x22, 0x0E,	/* PA[6]=0, PA[5]=0, PA[4]=0, PA[0]=0 */
	2, 0x26, 0xF1,	/* add */
	...
	2, 0x39, 0x08,	/* RGB_N_EQ1 */
	2, 0x3A, 0x12,	/* RGB_N_EQ2 */
	2, 0x3C, 0x78,	/* SET EQ3 */
	2, 0x3D, 0xFF,	/* SET CHGEN_OFF */
	2, 0x3E, 0xFF,	/* SET CHGEN_OFF */
	2, 0x3F, 0xFF,	/* SET CHGEN_OFF2 */

	/* Set TCON */
	2, 0x40, 0x06,	/* RSO=800 RGB */
	2, 0x41, 0xA0,	/* LN=640->1280 line */
	2, 0x4B, 0x04,	/* Source EQ iption, disable source EQ */

	/* --- power voltage  ---- */
	2, 0x55, 0x0F,	/* DCDCM=0001, JD PWR_IC */
	2, 0x56, 0x01,
	2, 0x57, 0x89,	/* VGH_RT=100, Charge pump Ratio:3*AVDD-VCL */
	2, 0x58, 0x0A,
	2, 0x59, 0x0A,	/* VCL = -2.9V */
	2, 0x5A, 0x28,	/* VGH = 15V */
	2, 0x5B, 0x14,	/* VGL = -11V */
	...
	/* GIP Timing */
	2, 0x58, 0x40,		/* GIP option for abnormal power off */
	2, 0x5B, 0x30,		/* STV number */
	2, 0x5C, 0x02,		/* STV shift */
	2, 0x5D, 0x40,		/* STV width */
	2, 0x5E, 0x01,		/* STV shift */
	2, 0x5F, 0x02,		/* STV shift */
	2, 0x63, 0x62,		/* STV on time */
	2, 0x64, 0x62,		/* STV off time */
	2, 0x67, 0x74,		/* CKV number & width */
	2, 0x68, 0x04,		/* CKV shift */
	2, 0x69, 0x62,		/* CKV on time */
	2, 0x6A, 0x66,		/* 66 CLK Falling to Source Falling time 3us */
	2, 0x6B, 0x08,		/* CKV dummy */
	2, 0x6C, 0x00,		/* GIP EQ off */
	2, 0x6D, 0x00,		/* GEQ_GGND1=0 */
	2, 0x6E, 0x00,		/* GEQ_GGND2=0 */
	2, 0x6F, 0x08,		/* GIP driving ability */

	/* Page1 */
	2, 0xE0, 0x01,
	2, 0x0E, 0x00,	/* LEDON output VCSW2 */

	/* Page3 */
	2, 0xE0, 0x03,
	2, 0x98, 0x3F,	/* From 2E to 2F, LED_VOL */
...
static int panel_ayf080r1140b01_prepare(struct drm_panel *panel)
{
	struct panel_ayf080r1140b01 *p = to_panel_ayf080r1140b01(panel);
	int err;
	...
	if (p->enable_gpio) {
		gpio_direction_output(p->enable_gpio,
			AYF_GPIO_ACTIVE(p->enable_gpio_flag));
		mdelay(50);
		gpio_direction_output(p->enable_gpio,
			AYF_GPIO_INACTIVE(p->enable_gpio_flag));
		mdelay(50);
	...
	ayf080r1140b01_mipi_dsi_dcs_write_array(&p->base,
		ayf080r1140b01_init_code,
		ARRAY_SIZE(ayf080r1140b01_init_code));
	msleep(150);

	return 0;
}

static int panel_ayf080r1140b01_enable(struct drm_panel *panel)
{
	struct panel_ayf080r1140b01 *p = to_panel_ayf080r1140b01(panel);
	...
	if (p->bl_gpio)
		gpio_direction_output(p->bl_gpio,
			AYF_GPIO_ACTIVE(p->bl_gpio_flag));

	return 0;
}

static int panel_ayf080r1140b01_get_fixed_modes(
	struct panel_ayf080r1140b01 *panel)
{
...
static int panel_ayf080r1140b01_get_modes(struct drm_panel *panel)
{
	struct panel_ayf080r1140b01 *p = to_panel_ayf080r1140b01(panel);
	int num = 0;
	...
	num += panel_ayf080r1140b01_get_fixed_modes(p);

	return num;
}

static int panel_ayf080r1140b01_get_timings(struct drm_panel *panel,
				    unsigned int num_timings,
...
static int panel_ayf080r1140b01_get_mode_private(
	struct drm_panel *panel,
...
static int panel_ayf080r1140b01_boot_sync(struct drm_panel *panel,
					int sync_step)
{
	struct panel_ayf080r1140b01 *p = to_panel_ayf080r1140b01(panel);
...
static const struct drm_panel_funcs panel_ayf080r1140b01_funcs = {
	.disable = panel_ayf080r1140b01_disable,
	.unprepare = panel_ayf080r1140b01_unprepare,
	.prepare = panel_ayf080r1140b01_prepare,
	.enable = panel_ayf080r1140b01_enable,
	.get_modes = panel_ayf080r1140b01_get_modes,
	.get_timings = panel_ayf080r1140b01_get_timings,
	.get_mode_private = panel_ayf080r1140b01_get_mode_private,
	.boot_sync = panel_ayf080r1140b01_boot_sync,
};

static int panel_ayf080r1140b01_brightness_map(
...
static int panel_ayf080r1140b01_backlight_update_status(
	struct backlight_device *bl)
{
	struct panel_ayf080r1140b01 *panel = bl_get_data(bl);
	struct pwm_device *bl_pwm = panel->bl_pwm;
	int brightness = bl->props.brightness;
	int period_ns = pwm_get_period(bl_pwm);

	if (brightness > 0) {
		int duty_cycle = panel_ayf080r1140b01_brightness_map(
...
static int panel_ayf080r1140b01_backlight_get_brightness(
	struct backlight_device *bl)
{
	return bl->props.brightness;
}

static const struct backlight_ops panel_ayf080r1140b01_backlight_ops = {
	.update_status	= panel_ayf080r1140b01_backlight_update_status,
	.get_brightness	= panel_ayf080r1140b01_backlight_get_brightness,
};

static int panel_ayf080r1140b01_probe(struct device *dev,
	const struct panel_ayf080r1140b01_desc *desc)
{
	struct device_node *np = dev->of_node;
	struct panel_ayf080r1140b01 *panel;
	struct panel_ayf080r1140b01_desc *of_desc;
	...
	if (IS_ERR_OR_NULL(panel->bl_pwm)) {
		...
		panel->backlight = backlight_device_register(
			"pwm-backlight",
			dev, panel,
			&panel_ayf080r1140b01_backlight_ops,
			&props);
	...
	panel->base.dev = dev;
	panel->base.funcs = &panel_ayf080r1140b01_funcs;
...
static int panel_ayf080r1140b01_remove(
	struct platform_device *pdev)
{
	struct panel_ayf080r1140b01 *panel = dev_get_drvdata(&pdev->dev);

	drm_panel_detach(&panel->base);
	drm_panel_remove(&panel->base);

	panel_ayf080r1140b01_disable(&panel->base);
	panel_ayf080r1140b01_unprepare(&panel->base);
...
static void panel_ayf080r1140b01_shutdown(
	struct platform_device *pdev)
{
	struct panel_ayf080r1140b01 *panel = dev_get_drvdata(&pdev->dev);
...
static struct lombo_vo_dev_config ayf080r1140b01_vo = {
	...
	.timing.field1.vt = AYF_VT,
	.timing.field1.vact = AYF_VACT,
	.timing.field1.vfp = AYF_VFP,
	.timing.field1.vsw = AYF_VSW,
	.timing.hline.ht = AYF_HSW + AYF_HBP + AYF_HACT + AYF_HFP,
	.timing.hline.hact = AYF_HACT,
	.timing.hline.hfp = AYF_HFP,
	.timing.hline.hsw = AYF_HSW,
	.timing.width = AYF_HACT,
	.timing.height = AYF_VACT,
	.timing.dclk_freq = AYF_DCLK,
	.format = TCON_FMT_RGB888,
};

/* mipi dsi 1080x1920 LCD test */
static const struct drm_display_mode ayf080r1140b01_mode = {
	/* dclk_freq */
	.clock = AYF_DCLK / 1000,
	/* width */
	.hdisplay = AYF_HACT,
	/* hsync_start = hdisplay + hfp */
	.hsync_start = AYF_HACT + AYF_HFP,
	/* hsync_end = hdisplay + hfp + hsw */
	.hsync_end = AYF_HACT + AYF_HFP + AYF_HSW,
	/* htotal = hdisplay + hfp + hsw + hbp */
	.htotal = AYF_HACT + AYF_HFP + AYF_HSW + AYF_HBP,
	/* height */
	.vdisplay = AYF_VACT,
	/* vsync_start = vdisplay + vfp */
	.vsync_start = AYF_VACT + AYF_VFP,
	/* vsync_end = vdisplay + vfp + vsw */
	.vsync_end = AYF_VACT + AYF_VFP + AYF_VSW,
	/* vtotal = vdisplay + vfp + vsw + vbp */
	.vtotal = AYF_VACT + AYF_VFP + AYF_VSW + AYF_VBP,
	.vrefresh = 60,

	.private_size = sizeof(ayf080r1140b01_vo),
	.private = (int *)&ayf080r1140b01_vo,
};

static const struct panel_ayf080r1140b01_desc ayf080r1140b01_desc = {
	.modes = &ayf080r1140b01_mode,
	.num_modes = 1,
	.bpc = 6,
	.size = {
		.width = 60,
...
static const struct of_device_id panel_ayf080r1140b01_of_match[] = {
	{
		.compatible = "hct,ayf080r1140b01",
		.data = &ayf080r1140b01_desc,
	}, {
		/* sentinel */
	}
};
MODULE_DEVICE_TABLE(of, panel_ayf080r1140b01_of_match);

static int panel_ayf080r1140b01_pre_probe(
	struct platform_device *pdev)
{
	...
	DRM_DEBUG_KMS("%s,%d\n", __FILE__, __LINE__);
	id = of_match_node(panel_ayf080r1140b01_of_match,
				pdev->dev.of_node);
	if (!id)
		return -ENODEV;

	return panel_ayf080r1140b01_probe(&pdev->dev, id->data);
}

static struct platform_driver panel_ayf080r1140b01_platform_driver = {
	.driver = {
		.name = "panel-ayf080r1140b01",
		.of_match_table = panel_ayf080r1140b01_of_match,
	},
	.probe = panel_ayf080r1140b01_pre_probe,
	.remove = panel_ayf080r1140b01_remove,
	.shutdown = panel_ayf080r1140b01_shutdown,
};

static int __init panel_ayf080r1140b01_init(void)
{
	int err;

	DRM_DEBUG_KMS("%s,%d\n", __FILE__, __LINE__);
	err = platform_driver_register(
		&panel_ayf080r1140b01_platform_driver);
	if (err < 0)
		return err;

	return 0;
}
fs_initcall(panel_ayf080r1140b01_init);

static void __exit panel_ayf080r1140b01_exit(void)
{
	platform_driver_unregister(&panel_ayf080r1140b01_platform_driver);
}
module_exit(panel_ayf080r1140b01_exit);

MODULE_AUTHOR("lomboswer <lomboswer@lombotech.com>");
MODULE_DESCRIPTION("DRM Driver for Panels ayf080r1140b01");
