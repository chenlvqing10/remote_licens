...
#define T070_HFP	(32)
#define T070_HSW	(20)
#define T070_HACT   (800)
#define T070_VBP	(6)
#define T070_VFP	(8)
#define T070_VSW	(4)
#define T070_VACT   (1280)
#define T070_VT	    (T070_VBP + T070_VFP + T070_VSW + T070_VACT)
#define T070_DCLK	(74250000)
...
struct panel_t070b056s1 {
	struct drm_panel base;
	bool prepared;
	bool enabled;

	struct device *dev;
	const struct panel_t070b056s1_desc *desc;
...
static inline struct panel_t070b056s1 *to_panel_t070b056s1(
	struct drm_panel *panel)
{
	return container_of(panel, struct panel_t070b056s1, base);
}

static int panel_t070b056s1_disable(struct drm_panel *panel)
{
	struct panel_t070b056s1 *p = to_panel_t070b056s1(panel);
...
static int panel_t070b056s1_unprepare(struct drm_panel *panel)
{
	struct panel_t070b056s1 *p = to_panel_t070b056s1(panel);
...
static int t070b056s1_mipi_dsi_dcs_write_array(
...
static u8 t070b056s1_init_code[] = {
	0x02, 0xE0, 0x00,
	0x02, 0xE1, 0x93,
	0x02, 0xE2, 0x65,
	0x02, 0xE3, 0xF8,
...
static int panel_t070b056s1_prepare(struct drm_panel *panel)
{
	struct panel_t070b056s1 *p = to_panel_t070b056s1(panel);
	int err;
	...
	t070b056s1_mipi_dsi_dcs_write_array(&p->base,
		t070b056s1_init_code,
		ARRAY_SIZE(t070b056s1_init_code));

	return 0;
}

static int panel_t070b056s1_enable(struct drm_panel *panel)
{
	struct panel_t070b056s1 *p = to_panel_t070b056s1(panel);
...
static int panel_t070b056s1_get_fixed_modes(
	struct panel_t070b056s1 *panel)
{
...
static int panel_t070b056s1_get_modes(struct drm_panel *panel)
{
	struct panel_t070b056s1 *p = to_panel_t070b056s1(panel);
	int num = 0;
	...
	num += panel_t070b056s1_get_fixed_modes(p);

	return num;
}

static int panel_t070b056s1_get_timings(struct drm_panel *panel,
				    unsigned int num_timings,
...
static int panel_t070b056s1_get_mode_private(
	struct drm_panel *panel,
...
static int panel_t070b056s1_boot_sync(struct drm_panel *panel,
					int sync_step)
{
	struct panel_t070b056s1 *p = to_panel_t070b056s1(panel);
...
static const struct drm_panel_funcs panel_t070b056s1_funcs = {
	.disable = panel_t070b056s1_disable,
	.unprepare = panel_t070b056s1_unprepare,
	.prepare = panel_t070b056s1_prepare,
	.enable = panel_t070b056s1_enable,
	.get_modes = panel_t070b056s1_get_modes,
	.get_timings = panel_t070b056s1_get_timings,
	.get_mode_private = panel_t070b056s1_get_mode_private,
	.boot_sync = panel_t070b056s1_boot_sync,
};

static int panel_t070b056s1_brightness_map(
...
static int panel_t070b056s1_backlight_update_status(
	struct backlight_device *bl)
{
	struct panel_t070b056s1 *panel = bl_get_data(bl);
	struct pwm_device *bl_pwm = panel->bl_pwm;
	int brightness = bl->props.brightness;
	int period_ns = pwm_get_period(bl_pwm);

	if (brightness > 0) {
		int duty_cycle = panel_t070b056s1_brightness_map(
...
static int panel_t070b056s1_backlight_get_brightness(
	struct backlight_device *bl)
{
	return bl->props.brightness;
}

static const struct backlight_ops panel_t070b056s1_backlight_ops = {
	.update_status	= panel_t070b056s1_backlight_update_status,
	.get_brightness	= panel_t070b056s1_backlight_get_brightness,
};

static int panel_t070b056s1_probe(struct device *dev,
	const struct panel_t070b056s1_desc *desc)
{
	struct device_node *np = dev->of_node;
	struct panel_t070b056s1 *panel;
	struct panel_t070b056s1_desc *of_desc;
	...
	if (IS_ERR_OR_NULL(panel->bl_pwm)) {
		...
		panel->backlight = backlight_device_register(
			"pwm-backlight",
			dev, panel,
			&panel_t070b056s1_backlight_ops,
			&props);
	...
	panel->base.dev = dev;
	panel->base.funcs = &panel_t070b056s1_funcs;
...
static int panel_t070b056s1_remove(
	struct platform_device *pdev)
{
	struct panel_t070b056s1 *panel = dev_get_drvdata(&pdev->dev);

	drm_panel_detach(&panel->base);
	drm_panel_remove(&panel->base);

	panel_t070b056s1_disable(&panel->base);
...
static void panel_t070b056s1_shutdown(
	struct platform_device *pdev)
{
	struct panel_t070b056s1 *panel = dev_get_drvdata(&pdev->dev);
...
static struct lombo_vo_dev_config t070b056s1_vo = {
	...
	.timing.field1.vt = T070_VT,
	.timing.field1.vact = T070_VACT,
	.timing.field1.vfp = T070_VFP,
	.timing.field1.vsw = T070_VSW,
	.timing.hline.ht = T070_HSW + T070_HBP + T070_HACT + T070_HFP,
	.timing.hline.hact = T070_HACT,
	.timing.hline.hfp = T070_HFP,
	.timing.hline.hsw = T070_HSW,
	.timing.width = T070_HACT,
	.timing.height = T070_VACT,
	.timing.dclk_freq = T070_DCLK,
	.format = TCON_FMT_RGB888,
};

/* mipi dsi 1080x1920 LCD test */
static const struct drm_display_mode t070b056s1_mode = {
	/* dclk_freq */
	.clock = T070_DCLK / 1000,
	/* width */
	.hdisplay = T070_HACT,
	/* hsync_start = hdisplay + hfp */
	.hsync_start = T070_HACT + T070_HFP,
	/* hsync_end = hdisplay + hfp + hsw */
	.hsync_end = T070_HACT + T070_HFP + T070_HSW,
	/* htotal = hdisplay + hfp + hsw + hbp */
	.htotal = T070_HACT + T070_HFP + T070_HSW + T070_HBP,
	/* height */
	.vdisplay = T070_VACT,
	/* vsync_start = vdisplay + vfp */
	.vsync_start = T070_VACT + T070_VFP,
	/* vsync_end = vdisplay + vfp + vsw */
	.vsync_end = T070_VACT + T070_VFP + T070_VSW,
	/* vtotal = vdisplay + vfp + vsw + vbp */
	.vtotal = T070_VACT + T070_VFP + T070_VSW + T070_VBP,
	.vrefresh = 60,

	.private_size = sizeof(t070b056s1_vo),
	.private = (int *)&t070b056s1_vo,
};

static const struct panel_t070b056s1_desc t070b056s1_desc = {
	.modes = &t070b056s1_mode,
	.num_modes = 1,
	.bpc = 6,
	.size = {
		.width = 60,
...
static const struct of_device_id panel_t070b056s1_of_match[] = {
	{
		.compatible = "fiti,t070b056s1",
		.data = &t070b056s1_desc,
	}, {
		/* sentinel */
	}
};
MODULE_DEVICE_TABLE(of, panel_t070b056s1_of_match);

static int panel_t070b056s1_pre_probe(
	struct platform_device *pdev)
{
	...
	DRM_DEBUG_KMS("%s,%d\n", __FILE__, __LINE__);
	id = of_match_node(panel_t070b056s1_of_match,
				pdev->dev.of_node);
	if (!id)
		return -ENODEV;

	return panel_t070b056s1_probe(&pdev->dev, id->data);
}

static struct platform_driver panel_t070b056s1_platform_driver = {
	.driver = {
		.name = "panel-t070b056s1",
		.of_match_table = panel_t070b056s1_of_match,
	},
	.probe = panel_t070b056s1_pre_probe,
	.remove = panel_t070b056s1_remove,
	.shutdown = panel_t070b056s1_shutdown,
};

static int __init panel_t070b056s1_init(void)
{
	int err;

	DRM_DEBUG_KMS("%s,%d\n", __FILE__, __LINE__);
	err = platform_driver_register(
		&panel_t070b056s1_platform_driver);
	if (err < 0)
		return err;

	return 0;
}
fs_initcall(panel_t070b056s1_init);

static void __exit panel_t070b056s1_exit(void)
{
	platform_driver_unregister(&panel_t070b056s1_platform_driver);
}
module_exit(panel_t070b056s1_exit);

MODULE_AUTHOR("lomboswer <lomboswer@lombotech.com>");
MODULE_DESCRIPTION("DRM Driver for Panels t070b056s1");
