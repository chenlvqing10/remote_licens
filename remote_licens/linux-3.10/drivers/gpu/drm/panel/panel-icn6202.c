...
#define ICN6202_LANE_NUM	(4)

#define ICN6202_WIDTH		(1280)
#define ICN6202_HEIGHT		(800)

#define ICN6202_HBP		(80)
#define ICN6202_HFP		(48)
#define ICN6202_HSW		(32)
#define ICN6202_HACT		(ICN6202_WIDTH)

#define ICN6202_VBP_F1		(14)
#define ICN6202_VFP_F1		(3)
#define ICN6202_VSW_F1		(6)
#define ICN6202_VACT_F1		(ICN6202_HEIGHT)

#define ICN6202_VBP_F2		(0)
#define ICN6202_VFP_F2		(0)
#define ICN6202_VSW_F2		(0)
#define ICN6202_VACT_F2		(0)

#define ICN6202_DCLK		(74250000)

#define ICN6202_DCS_MDELAY_FLAG (0)
...
struct panel_icn6202 {
	struct drm_panel base;
	bool prepared;
	bool enabled;

	struct device *dev;
	const struct panel_icn6202_desc *desc;
...
/* only 1 i2c addr */
static struct i2c_client *panel_icn6202_i2c_client[1];

u8 icn6202_init_code[] = {
	3, 0x5a, 0x20, 0x00,
	3, 0x5a, 0x21, 0x20,
	3, 0x5a, 0x22, 0x35,
	3, 0x5a, 0x23, 0x30,
...
static struct i2c_client *panel_icn6202_get_i2c_client(
					const unsigned short i2c_addr)
{
	const __u32 num = ARRAY_SIZE(panel_icn6202_i2c_client);
	__u32 i;

	for (i = 0; i < num; i++) {
		struct i2c_client *client = panel_icn6202_i2c_client[i];
...
s32 panel_icn6202_i2c_write_reg_array(
				u32 tidx, u8 *reg, u32 len)
{
	...
	struct i2c_client *client =
			panel_icn6202_get_i2c_client(reg[1] >> 1);
	read_buf[0] = 0;
	addr[0] = 0;

	if (1 != i2c_master_send(client, addr, 1)) {
		DRM_ERROR("%d\n", __LINE__);
		return -1;
	}
	...
	DRM_DEBUG_KMS("read[client0x%x][addr=0x%x], read_buf=%x\n",
		reg[1],	addr[0], read_buf[0]);
#endif


	while (i < len) {
		if (ICN6202_DCS_MDELAY_FLAG == reg[i]) {
			mdelay(reg[i + 1]);
			i += reg[i] + 2;
		} else {
			char buf[3];
			struct i2c_client *client =
				panel_icn6202_get_i2c_client(reg[i + 1] >> 1);
...
static inline struct panel_icn6202 *to_panel_icn6202(
	struct drm_panel *panel)
{
	return container_of(panel, struct panel_icn6202, base);
}

static int panel_icn6202_disable(struct drm_panel *panel)
{
	struct panel_icn6202 *p = to_panel_icn6202(panel);
...
static int panel_icn6202_unprepare(struct drm_panel *panel)
{
	struct panel_icn6202 *p = to_panel_icn6202(panel);
...
static int panel_icn6202_prepare(struct drm_panel *panel)
{
	struct panel_icn6202 *p = to_panel_icn6202(panel);
	int err;
...
static int panel_icn6202_enable(struct drm_panel *panel)
{
	struct panel_icn6202 *p = to_panel_icn6202(panel);
	...
	msleep(100);
	panel_icn6202_i2c_write_reg_array(3, icn6202_init_code,
					  ARRAY_SIZE(icn6202_init_code));
...
static int panel_icn6202_get_fixed_modes(
	struct panel_icn6202 *panel)
{
...
static int panel_icn6202_get_modes(struct drm_panel *panel)
{
	struct panel_icn6202 *p = to_panel_icn6202(panel);
	int num = 0;
	...
	num += panel_icn6202_get_fixed_modes(p);

	return num;
}

static int panel_icn6202_get_timings(struct drm_panel *panel,
				    unsigned int num_timings,
...
static int panel_icn6202_get_mode_private(
	struct drm_panel *panel,
...
static int panel_icn6202_boot_sync(struct drm_panel *panel,
					int sync_step)
{
	struct panel_icn6202 *p = to_panel_icn6202(panel);
...
static int panel_icn6202_i2c_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	panel_icn6202_i2c_client[0] = client;
	for (i = 1; i < ARRAY_SIZE(panel_icn6202_i2c_client); i++) {
		panel_icn6202_i2c_client[i] = i2c_new_dummy(client->adapter,
					client->addr + i);
		if (!panel_icn6202_i2c_client[i]) {
			...
			dev_info(&client->adapter->dev,
				...
				client->addr + i,
				panel_icn6202_i2c_client[i]->addr);
		}
	}

	dev_info(&client->adapter->dev,
		"attached panel icn6202 into i2c adapter successfully\n");
	return 0;
}

static int panel_icn6202_i2c_remove(struct i2c_client *client)
{
	dev_info(&client->adapter->dev,
		"detached panel icn6202 from i2c adapter successfully\n");

	return 0;
}

static const struct i2c_device_id panel_icn6202_i2c_id[] = {
	{ "icn6202_i2c", 0 },
	{ },
};

static struct of_device_id panel_icn6202_i2c_match_types[] = {
	{
...
struct i2c_driver panel_icn6202_i2c_driver = {
	.driver = {
		.name = "icn6202_i2c",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(panel_icn6202_i2c_match_types),
	},
	.id_table = panel_icn6202_i2c_id,
	.probe = panel_icn6202_i2c_probe,
	.remove = panel_icn6202_i2c_remove,
	.command = NULL,
};

static const struct drm_panel_funcs panel_icn6202_funcs = {
	.disable = panel_icn6202_disable,
	.unprepare = panel_icn6202_unprepare,
	.prepare = panel_icn6202_prepare,
	.enable = panel_icn6202_enable,
	.get_modes = panel_icn6202_get_modes,
	.get_timings = panel_icn6202_get_timings,
	.get_mode_private = panel_icn6202_get_mode_private,
	.boot_sync = panel_icn6202_boot_sync,
};

static int panel_icn6202_brightness_map(
...
static int panel_icn6202_backlight_update_status(
	struct backlight_device *bl)
{
	struct panel_icn6202 *panel = bl_get_data(bl);
	struct pwm_device *bl_pwm = panel->bl_pwm;
	int brightness = bl->props.brightness;
	int period_ns = pwm_get_period(bl_pwm);

	if (brightness > 0) {
		int duty_cycle = panel_icn6202_brightness_map(
...
static int panel_icn6202_backlight_get_brightness(
	struct backlight_device *bl)
{
	return bl->props.brightness;
}

static const struct backlight_ops panel_icn6202_backlight_ops = {
	.update_status	= panel_icn6202_backlight_update_status,
	.get_brightness	= panel_icn6202_backlight_get_brightness,
};

static int panel_icn6202_probe(struct device *dev,
			       const struct panel_icn6202_desc *desc)
{
	struct device_node *np = dev->of_node;
	struct panel_icn6202 *panel;
	struct panel_icn6202_desc *of_desc;
	...
	if (i2c_add_driver(&panel_icn6202_i2c_driver)) {
	...
	panel->base.dev = dev;
	panel->base.funcs = &panel_icn6202_funcs;
...
static int panel_icn6202_remove(struct platform_device *pdev)
{
	struct panel_icn6202 *panel = dev_get_drvdata(&pdev->dev);

	drm_panel_detach(&panel->base);
	drm_panel_remove(&panel->base);

	panel_icn6202_disable(&panel->base);
	panel_icn6202_unprepare(&panel->base);
...
static void panel_icn6202_shutdown(struct platform_device *pdev)
{
	struct panel_icn6202 *panel = dev_get_drvdata(&pdev->dev);
...
static struct lombo_vo_dev_config icn6202_vo = {
	...
	.timing.field1.vt = ICN6202_VBP_F1 + ICN6202_VFP_F1 +
					ICN6202_VSW_F1 + ICN6202_VACT_F1,
	.timing.field1.vact = ICN6202_VACT_F1,
	.timing.field1.vfp = ICN6202_VFP_F1,
	.timing.field1.vsw = ICN6202_VSW_F1,
	.timing.field2.vt = ICN6202_VBP_F2 + ICN6202_VFP_F2 +
					ICN6202_VSW_F2 + ICN6202_VACT_F2,
	.timing.field2.vact = ICN6202_VACT_F2,
	.timing.field2.vfp = ICN6202_VFP_F2,
	.timing.field2.vsw = ICN6202_VSW_F2,
	.timing.hline.ht = ICN6202_HSW + ICN6202_HBP +
				ICN6202_HACT + ICN6202_HFP,
	.timing.hline.hact = ICN6202_HACT,
	.timing.hline.hfp = ICN6202_HFP,
	.timing.hline.hsw = ICN6202_HSW,
	.timing.width = ICN6202_WIDTH,
	.timing.height = ICN6202_HEIGHT,
	.timing.dclk_freq = ICN6202_DCLK,
	.format = TCON_FMT_RGB888,
};

static const struct drm_display_mode icn6202_mode = {
	/* dclk_freq */
	.clock = ICN6202_DCLK / 1000,
	/* width */
	.hdisplay = ICN6202_WIDTH,
	/* hsync_start = hdisplay + hfp */
	.hsync_start = ICN6202_WIDTH + ICN6202_HFP,
	/* hsync_end = hdisplay + hfp + hsw */
	.hsync_end = ICN6202_WIDTH + ICN6202_HFP + ICN6202_HSW,
	/* htotal = hdisplay + hfp + hsw + hbp */
	.htotal = ICN6202_WIDTH + ICN6202_HFP + ICN6202_HSW + ICN6202_HBP,
	/* height */
	.vdisplay = ICN6202_HEIGHT,
	/* vsync_start = vdisplay + vfp */
	.vsync_start = ICN6202_HEIGHT + ICN6202_VFP_F1,
	/* vsync_end = vdisplay + vfp + vsw */
	.vsync_end = ICN6202_HEIGHT + ICN6202_VFP_F1 + ICN6202_VSW_F1,
	/* vtotal = vdisplay + vfp + vsw + vbp */
	.vtotal = ICN6202_HEIGHT + ICN6202_VFP_F1 +
			ICN6202_VSW_F1 + ICN6202_VBP_F1,
	.vrefresh = 60,

	.private_size = sizeof(icn6202_vo),
	.private = (int *)&icn6202_vo,
};

static const struct panel_icn6202_desc icn6202_desc = {
	.modes = &icn6202_mode,
	.num_modes = 1,
	.bpc = 6,
	.size = {
...
static const struct of_device_id panel_icn6202_of_match[] = {
	{
...
MODULE_DEVICE_TABLE(of, panel_icn6202_of_match);

static int panel_icn6202_pre_probe(struct platform_device *pdev)
{
	...
	DRM_DEBUG_KMS("%s,%d\n", __FILE__, __LINE__);
	id = of_match_node(panel_icn6202_of_match, pdev->dev.of_node);
	if (!id)
		return -ENODEV;

	return panel_icn6202_probe(&pdev->dev, id->data);
}

static struct platform_driver panel_icn6202_platform_driver = {
	.driver = {
		.name = "panel-icn6202",
		.of_match_table = panel_icn6202_of_match,
	},
	.probe = panel_icn6202_pre_probe,
	.remove = panel_icn6202_remove,
	.shutdown = panel_icn6202_shutdown,
};

static int __init panel_icn6202_init(void)
{
	int err;

	DRM_DEBUG_KMS("%s,%d\n", __FILE__, __LINE__);
	err = platform_driver_register(&panel_icn6202_platform_driver);
	if (err < 0)
		return err;

	return 0;
}
fs_initcall(panel_icn6202_init);

static void __exit panel_icn6202_exit(void)
{
	platform_driver_unregister(&panel_icn6202_platform_driver);
}
module_exit(panel_icn6202_exit);

MODULE_AUTHOR("lomboswer <lomboswer@lombotech.com>");
MODULE_DESCRIPTION("DRM Driver for Panels icn6202");
