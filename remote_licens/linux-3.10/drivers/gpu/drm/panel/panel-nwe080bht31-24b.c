...
#define NWE080_GPIO_ACTIVE(flag) (((flag) & OF_GPIO_ACTIVE_LOW) ? 0 : 1)
#define NWE080_GPIO_INACTIVE(flag) (((flag) & OF_GPIO_ACTIVE_LOW) ? 1 : 0)

/* timing for tcon host */
#define NWE080_HBP	(20)
#define NWE080_HFP	(32)//16
#define NWE080_HSW	(20)
#define NWE080_HACT	(800)
#define NWE080_HT	(NWE080_HSW + NWE080_HBP + NWE080_HACT + NWE080_HFP)

#define NWE080_VBP	(4)
#define NWE080_VFP	(8)
#define NWE080_VSW	(4)
#define NWE080_VACT	(1280)
#define NWE080_VT	(NWE080_VBP + NWE080_VFP + NWE080_VSW + NWE080_VACT)

#define NWE080_DCLK	(65000000)//74250000
...
struct panel_nwe080 {
	...
	struct device *dev;
	const struct panel_nwe080_desc *desc;
...
static inline struct panel_nwe080 *to_panel_nwe080(
	struct drm_panel *panel)
{
	return container_of(panel, struct panel_nwe080, base);
}

static int panel_nwe080_disable(struct drm_panel *panel)
{
	struct panel_nwe080 *p = to_panel_nwe080(panel);
	...
	if (p->bl_gpio)
		gpio_direction_output(p->bl_gpio,
			NWE080_GPIO_INACTIVE(p->bl_gpio_flag));
...
static int panel_nwe080_unprepare(struct drm_panel *panel)
{
	struct panel_nwe080 *p = to_panel_nwe080(panel);
	...
	if (p->enable_gpio)
		gpio_direction_output(p->enable_gpio,
			NWE080_GPIO_INACTIVE(p->enable_gpio_flag));
...
static int panel_nwe080_mipi_dsi_dcs_write_array(
...
static u8 nwe080_init_code[] = {
	...
	//--- power voltage  ----//
	2, 0x55, 0x01,	//DCDCM=0001,SPI_WriteData( JD PWR_IC
	2, 0x56, 0x01,
	2, 0x57, 0x69,
	2, 0x58, 0x0A,
	2, 0x59, 0x0A,
	...
	//Page1
	2, 0xE0, 0x01,
	2, 0x0E, 0x01,	//LEDON output VCSW2
	
	//Page3
	2, 0xE0, 0x03,
	2, 0x98, 0x2F,	//From 2E to 2F  LED_VOL
...
static int panel_nwe080_prepare(struct drm_panel *panel)
{
	struct panel_nwe080 *p = to_panel_nwe080(panel);
	int err;
	...
	if (p->enable_gpio) {
		...
		gpio_direction_output(p->enable_gpio,
			NWE080_GPIO_ACTIVE(p->enable_gpio_flag));
		mdelay(100);
		gpio_direction_output(p->enable_gpio,
			NWE080_GPIO_INACTIVE(p->enable_gpio_flag));
		mdelay(100);
	...
	panel_nwe080_mipi_dsi_dcs_write_array(&p->base,
		nwe080_init_code,
		ARRAY_SIZE(nwe080_init_code));

	return 0;
}

static int panel_nwe080_enable(struct drm_panel *panel)
{
	struct panel_nwe080 *p = to_panel_nwe080(panel);
	...
	if (p->bl_gpio)
		gpio_direction_output(p->bl_gpio,
			NWE080_GPIO_ACTIVE(p->bl_gpio_flag));

	return 0;
}

static int panel_nwe080_get_fixed_modes(
	struct panel_nwe080 *panel)
{
...
static int panel_nwe080_get_modes(struct drm_panel *panel)
{
	struct panel_nwe080 *p = to_panel_nwe080(panel);
	int num = 0;
	...
	num += panel_nwe080_get_fixed_modes(p);

	return num;
}

static int panel_nwe080_get_timings(struct drm_panel *panel,
				    unsigned int num_timings,
...
static int panel_nwe080_get_mode_private(
	struct drm_panel *panel,
...
static int panel_nwe080_boot_sync(struct drm_panel *panel,
					int sync_step)
{
	struct panel_nwe080 *p = to_panel_nwe080(panel);
...
static const struct drm_panel_funcs panel_nwe080_funcs = {
	.disable = panel_nwe080_disable,
	.unprepare = panel_nwe080_unprepare,
	.prepare = panel_nwe080_prepare,
	.enable = panel_nwe080_enable,
	.get_modes = panel_nwe080_get_modes,
	.get_timings = panel_nwe080_get_timings,
	.get_mode_private = panel_nwe080_get_mode_private,
	.boot_sync = panel_nwe080_boot_sync,
};

static int panel_nwe080_brightness_map(
...
static int panel_nwe080_backlight_update_status(
	struct backlight_device *bl)
{
	struct panel_nwe080 *panel = bl_get_data(bl);
	struct pwm_device *bl_pwm = panel->bl_pwm;
	int brightness = bl->props.brightness;
	int period_ns = pwm_get_period(bl_pwm);

	if (brightness > 0) {
		int duty_cycle = panel_nwe080_brightness_map(
...
static int panel_nwe080_backlight_get_brightness(
	struct backlight_device *bl)
{
	return bl->props.brightness;
}

static const struct backlight_ops panel_nwe080_backlight_ops = {
	.update_status	= panel_nwe080_backlight_update_status,
	.get_brightness	= panel_nwe080_backlight_get_brightness,
};

static int panel_nwe080_probe(struct device *dev,
	const struct panel_nwe080_desc *desc)
{
	struct device_node *np = dev->of_node;
	struct panel_nwe080 *panel;
	struct panel_nwe080_desc *of_desc;
	...
	panel->base.dev = dev;
	panel->base.funcs = &panel_nwe080_funcs;
...
static int panel_nwe080_remove(
	struct platform_device *pdev)
{
	struct panel_nwe080 *panel = dev_get_drvdata(&pdev->dev);

	drm_panel_detach(&panel->base);
	drm_panel_remove(&panel->base);

	panel_nwe080_disable(&panel->base);
...
static void panel_nwe080_shutdown(
	struct platform_device *pdev)
{
	struct panel_nwe080 *panel = dev_get_drvdata(&pdev->dev);
...
static struct lombo_vo_dev_config nwe080_vo = {
	...
	.timing.field1.vt = NWE080_VT,
	.timing.field1.vact = NWE080_VACT,
	.timing.field1.vfp = NWE080_VFP,
	.timing.field1.vsw = NWE080_VSW,
	.timing.hline.ht = NWE080_HT,
	.timing.hline.hact = NWE080_HACT,
	.timing.hline.hfp = NWE080_HFP,
	.timing.hline.hsw = NWE080_HSW,
	.timing.width = NWE080_HACT,
	.timing.height = NWE080_VACT,
	.timing.dclk_freq = NWE080_DCLK,
	.format = TCON_FMT_RGB888,
};

/* mipi dsi 1080x1920 LCD test */
static const struct drm_display_mode nwe080_mode = {
	/* dclk_freq */
	.clock = NWE080_DCLK / 1000,
	/* width */
	.hdisplay = NWE080_HACT,
	/* hsync_start = hdisplay + hfp */
	.hsync_start = NWE080_HACT + NWE080_HFP,
	/* hsync_end = hdisplay + hfp + hsw */
	.hsync_end = NWE080_HACT + NWE080_HFP + NWE080_HSW,
	/* htotal = hdisplay + hfp + hsw + hbp */
	.htotal = NWE080_HACT + NWE080_HFP + NWE080_HSW + NWE080_HBP,
	/* height */
	.vdisplay = NWE080_VACT,
	/* vsync_start = vdisplay + vfp */
	.vsync_start = NWE080_VACT + NWE080_VFP,
	/* vsync_end = vdisplay + vfp + vsw */
	.vsync_end = NWE080_VACT + NWE080_VFP + NWE080_VSW,
	/* vtotal = vdisplay + vfp + vsw + vbp */
	.vtotal = NWE080_VACT + NWE080_VFP + NWE080_VSW + NWE080_VBP,
	.vrefresh = 60,

	.private_size = sizeof(nwe080_vo),
	.private = (int *)&nwe080_vo,
};

static const struct panel_nwe080_desc nwe080_desc = {
	.modes = &nwe080_mode,
	.num_modes = 1,
	.bpc = 6,
	.size = {
...
static const struct of_device_id panel_nwe080_of_match[] = {
	{
		.compatible = "fiti,nwe080bht31-24b",
		.data = &nwe080_desc,
	}, {
		/* sentinel */
	}
};
MODULE_DEVICE_TABLE(of, panel_nwe080_of_match);

static int panel_nwe080_pre_probe(
	struct platform_device *pdev)
{
	...
	DRM_DEBUG_KMS("%s,%d\n", __FILE__, __LINE__);
	id = of_match_node(panel_nwe080_of_match,
				pdev->dev.of_node);
	if (!id)
		return -ENODEV;

	return panel_nwe080_probe(&pdev->dev, id->data);
}

static struct platform_driver panel_nwe080_platform_driver = {
	.driver = {
		.name = "panel-nwe080",
		.of_match_table = panel_nwe080_of_match,
	},
	.probe = panel_nwe080_pre_probe,
	.remove = panel_nwe080_remove,
	.shutdown = panel_nwe080_shutdown,
};

static int __init panel_nwe080_init(void)
{
	int err;

	DRM_DEBUG_KMS("%s,%d\n", __FILE__, __LINE__);
	err = platform_driver_register(
		&panel_nwe080_platform_driver);
	if (err < 0)
		return err;

	return 0;
}
fs_initcall(panel_nwe080_init);

static void __exit panel_nwe080_exit(void)
{
	platform_driver_unregister(&panel_nwe080_platform_driver);
}
module_exit(panel_nwe080_exit);

MODULE_AUTHOR("lomboswer <lomboswer@lombotech.com>");
MODULE_DESCRIPTION("DRM Driver for Panels nwe080bht31-24b");
