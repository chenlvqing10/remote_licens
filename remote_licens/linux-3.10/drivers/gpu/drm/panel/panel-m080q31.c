...
#define M080_GPIO_ACTIVE(flag) (((flag) & OF_GPIO_ACTIVE_LOW) ? 0 : 1)
#define M080_GPIO_INACTIVE(flag) (((flag) & OF_GPIO_ACTIVE_LOW) ? 1 : 0)

#define M080_WIDTH	(800)
#define M080_HEIGHT	(1280)

/* timing for tcon host */
#define M080_HBP	(50)
#define M080_HFP	(50)
#define M080_HSW	(28)
#define M080_HACT	(M080_WIDTH)
#define M080_VBP	(20)
#define M080_VFP	(24)
#define M080_VSW	(8)
#define M080_VACT	(M080_HEIGHT)
#define M080_VT	(M080_VBP + M080_VFP + M080_VSW + M080_VACT)
#define M080_DCLK	(74250000)
...
struct panel_m080q31 {
	struct drm_panel base;
	bool prepared;
	bool enabled;

	struct device *dev;
	const struct panel_m080q31_desc *desc;
...
static inline struct panel_m080q31 *to_panel_m080q31(
	struct drm_panel *panel)
{
	return container_of(panel, struct panel_m080q31, base);
}

static int panel_m080q31_disable(struct drm_panel *panel)
{
	struct panel_m080q31 *p = to_panel_m080q31(panel);
	...
	if (p->bl_gpio)
		gpio_direction_output(p->bl_gpio,
			M080_GPIO_INACTIVE(p->bl_gpio_flag));
...
static int panel_m080q31_unprepare(struct drm_panel *panel)
{
	struct panel_m080q31 *p = to_panel_m080q31(panel);
	...
	if (p->enable_gpio)
		gpio_direction_output(p->enable_gpio,
			M080_GPIO_INACTIVE(p->enable_gpio_flag));
...
static int m080q31_mipi_dsi_dcs_write_array(
...
static int panel_m080q31_prepare(struct drm_panel *panel)
{
	struct panel_m080q31 *p = to_panel_m080q31(panel);
	int err;
	...
	if (p->enable_gpio) {
		gpio_direction_output(p->enable_gpio,
			M080_GPIO_ACTIVE(p->enable_gpio_flag));
		mdelay(10);
		gpio_direction_output(p->enable_gpio,
			M080_GPIO_INACTIVE(p->enable_gpio_flag));
		mdelay(10);
	...
	m080q31_mipi_dsi_dcs_write_array(&p->base,
		m080q31_init_code,
		ARRAY_SIZE(m080q31_init_code));
	msleep(150);

	return 0;
}

static int panel_m080q31_enable(struct drm_panel *panel)
{
	struct panel_m080q31 *p = to_panel_m080q31(panel);
	...
	if (p->bl_gpio)
		gpio_direction_output(p->bl_gpio,
			M080_GPIO_ACTIVE(p->bl_gpio_flag));

	return 0;
}

static int panel_m080q31_get_fixed_modes(
	struct panel_m080q31 *panel)
{
...
static int panel_m080q31_get_modes(struct drm_panel *panel)
{
	struct panel_m080q31 *p = to_panel_m080q31(panel);
	int num = 0;
	...
	num += panel_m080q31_get_fixed_modes(p);

	return num;
}

static int panel_m080q31_get_timings(struct drm_panel *panel,
				    unsigned int num_timings,
...
static int panel_m080q31_get_mode_private(
	struct drm_panel *panel,
...
static int panel_m080q31_boot_sync(struct drm_panel *panel,
					int sync_step)
{
	struct panel_m080q31 *p = to_panel_m080q31(panel);
...
static const struct drm_panel_funcs panel_m080q31_funcs = {
	.disable = panel_m080q31_disable,
	.unprepare = panel_m080q31_unprepare,
	.prepare = panel_m080q31_prepare,
	.enable = panel_m080q31_enable,
	.get_modes = panel_m080q31_get_modes,
	.get_timings = panel_m080q31_get_timings,
	.get_mode_private = panel_m080q31_get_mode_private,
	.boot_sync = panel_m080q31_boot_sync,
};

static int panel_m080q31_brightness_map(
...
static int panel_m080q31_backlight_update_status(
	struct backlight_device *bl)
{
	struct panel_m080q31 *panel = bl_get_data(bl);
	struct pwm_device *bl_pwm = panel->bl_pwm;
	int brightness = bl->props.brightness;
	int period_ns = pwm_get_period(bl_pwm);

	if (brightness > 0) {
		int duty_cycle = panel_m080q31_brightness_map(
...
static int panel_m080q31_backlight_get_brightness(
	struct backlight_device *bl)
{
	return bl->props.brightness;
}

static const struct backlight_ops panel_m080q31_backlight_ops = {
	.update_status	= panel_m080q31_backlight_update_status,
	.get_brightness	= panel_m080q31_backlight_get_brightness,
};

static int panel_m080q31_probe(struct device *dev,
	const struct panel_m080q31_desc *desc)
{
	struct device_node *np = dev->of_node;
	struct panel_m080q31 *panel;
	struct panel_m080q31_desc *of_desc;
	...
	panel->base.dev = dev;
	panel->base.funcs = &panel_m080q31_funcs;
...
static int panel_m080q31_remove(
	struct platform_device *pdev)
{
	struct panel_m080q31 *panel = dev_get_drvdata(&pdev->dev);

	drm_panel_detach(&panel->base);
	drm_panel_remove(&panel->base);

	panel_m080q31_disable(&panel->base);
	panel_m080q31_unprepare(&panel->base);
...
static void panel_m080q31_shutdown(
	struct platform_device *pdev)
{
	struct panel_m080q31 *panel = dev_get_drvdata(&pdev->dev);
...
static struct lombo_vo_dev_config m080q31_vo = {
	...
	.timing.field1.vt = M080_VT,
	.timing.field1.vact = M080_VACT,
	.timing.field1.vfp = M080_VFP,
	.timing.field1.vsw = M080_VSW,
	.timing.hline.ht = M080_HSW + M080_HBP + M080_HACT + M080_HFP,
	.timing.hline.hact = M080_HACT,
	.timing.hline.hfp = M080_HFP,
	.timing.hline.hsw = M080_HSW,
	.timing.width = M080_HACT,
	.timing.height = M080_VACT,
	.timing.dclk_freq = M080_DCLK,
	.format = TCON_FMT_RGB888,
};

/* mipi dsi 1080x1920 LCD test */
static const struct drm_display_mode m080q31_mode = {
	/* dclk_freq */
	.clock = M080_DCLK / 1000,
	/* width */
	.hdisplay = M080_HACT,
	/* hsync_start = hdisplay + hfp */
	.hsync_start = M080_HACT + M080_HFP,
	/* hsync_end = hdisplay + hfp + hsw */
	.hsync_end = M080_HACT + M080_HFP + M080_HSW,
	/* htotal = hdisplay + hfp + hsw + hbp */
	.htotal = M080_HACT + M080_HFP + M080_HSW + M080_HBP,
	/* height */
	.vdisplay = M080_VACT,
	/* vsync_start = vdisplay + vfp */
	.vsync_start = M080_VACT + M080_VFP,
	/* vsync_end = vdisplay + vfp + vsw */
	.vsync_end = M080_VACT + M080_VFP + M080_VSW,
	/* vtotal = vdisplay + vfp + vsw + vbp */
	.vtotal = M080_VACT + M080_VFP + M080_VSW + M080_VBP,
	.vrefresh = 60,

	.private_size = sizeof(m080q31_vo),
	.private = (int *)&m080q31_vo,
};

static const struct panel_m080q31_desc m080q31_desc = {
	.modes = &m080q31_mode,
	.num_modes = 1,
	.bpc = 6,
	.size = {
...
static const struct of_device_id panel_m080q31_of_match[] = {
	{
		.compatible = "m080q31",
		.data = &m080q31_desc,
	}, {
...
MODULE_DEVICE_TABLE(of, panel_m080q31_of_match);

static int panel_m080q31_pre_probe(
	struct platform_device *pdev)
{
	...
	DRM_DEBUG_KMS("%s,%d\n", __FILE__, __LINE__);
	id = of_match_node(panel_m080q31_of_match,
				pdev->dev.of_node);
	if (!id)
		return -ENODEV;

	return panel_m080q31_probe(&pdev->dev, id->data);
}

static struct platform_driver panel_m080q31_platform_driver = {
	.driver = {
		.name = "panel-m080q31",
		.of_match_table = panel_m080q31_of_match,
	},
	.probe = panel_m080q31_pre_probe,
	.remove = panel_m080q31_remove,
	.shutdown = panel_m080q31_shutdown,
};

static int __init panel_m080q31_init(void)
{
	int err;

	DRM_DEBUG_KMS("%s,%d\n", __FILE__, __LINE__);
	err = platform_driver_register(
		&panel_m080q31_platform_driver);
	if (err < 0)
		return err;

	return 0;
}
fs_initcall(panel_m080q31_init);

static void __exit panel_m080q31_exit(void)
{
	platform_driver_unregister(&panel_m080q31_platform_driver);
}
module_exit(panel_m080q31_exit);

MODULE_AUTHOR("lomboswer <lomboswer@lombotech.com>");
MODULE_DESCRIPTION("DRM Driver for Panels m080q31");
