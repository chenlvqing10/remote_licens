...
bool drm_fb_helper_destory_fbdev_mode(struct drm_fb_helper *fb_helper)
{
	bool error = false;
	int i, ret;
	...
	for (i = 0; i < fb_helper->crtc_count; i++) {
		struct drm_mode_set set = fb_helper->crtc_info[i].mode_set;
...
EXPORT_SYMBOL(drm_fb_helper_destory_fbdev_mode);
		     ...
		     u16 blue, u16 regno, struct fb_info *info)
{
	...
	if (!fb_helper->funcs->gamma_set)
		return -EINVAL;

	pindex = regno;

	if (fb->format->cpp[0] == 2) {
		pindex = regno << 3;

		if (fb->format->depth == 16 && regno > 63)
			return -EINVAL;
		if (fb->format->depth == 15 && regno > 31)
			return -EINVAL;
...
int drm_fb_helper_set_par(struct fb_info *info)
{
	...
	drm_modeset_lock_all(dev);
	for (i = 0; i < fb_helper->crtc_count; i++) {
		fb_helper->crtc_info[i].mode_set.fb_dirty = 1;
		ret = drm_mode_set_config_internal(&fb_helper->crtc_info[i].mode_set);
		fb_helper->crtc_info[i].mode_set.fb_dirty = 0;
		if (ret) {
			drm_modeset_unlock_all(dev);
			return ret;
		}
...
int drm_fb_helper_hotplug_event(struct drm_fb_helper *fb_helper)
{
	...
	bpp_sel = fb_helper->fb->format->cpp[0] * 8;
