 ...
 * VO_DEV_TVE
 */
#define INTERFACE	(VO_DEV_LVDS)
#define IS_DUAL_LINK	(FALSE)
#define IS_CTRL_INVERT	(FALSE) /* TODO */

/*
 * @LVDS_MODE
 * TCON_LVDS_JEIDA, TCON_LVDS_VESA
 */
#define LVDS_MODE	(TCON_LVDS_VESA)
...
static void tcon_dev_config(vo_device_t *vo_dev)
{
	...
	vo_dev->if_cfg.lvds_if.is_dual_link = IS_DUAL_LINK;
	vo_dev->if_cfg.lvds_if.is_ctrl_invert = IS_CTRL_INVERT; /* TODO */
	vo_dev->if_cfg.lvds_if.mode = LVDS_MODE;
	vo_dev->if_cfg.lvds_if.bpp = COLOR_DEPTH;
...
static void doss_set_bl_pwm_io(u32 doss_idx, u32 val)
{
	...
	/* reg_addr = (u32 *)0xf5014124; */
	reg_addr = (u32 *)0xf4015124;
...
static void doss_set_bl_pwm_cycle(u32 doss_idx, u32 cycle)
{
	...
	rdval = READREG32(reg_addr);
	DRM_DEBUG_KMS("[%d] rdval:0x%x\n", __LINE__, rdval);
	WRITEREG32(reg_addr, (rdval & 0xfffffff0) | 0x00000005);
	rdval = READREG32(reg_addr);
	DRM_DEBUG_KMS("[%d] rdval:0x%x\n", __LINE__, rdval);
	pr_debug("%s [%d] rdval:0x%x\n", __func__, __LINE__, rdval);

	pwm_cycle = 100 / cycle - 1;
	pr_debug("%s [%d] pwm_cycle:0x%x\n", __func__, __LINE__, pwm_cycle);

	/* 50% pwm cycle */
	WRITEREG32(0xf400d000 + 0x004, 0x100);
	pr_debug("%s [%d] 0xf400d000 + 0x004:0x%x\n",
		__func__, __LINE__, READREG32(0xf400d000 + 0x004));

	WRITEREG32(0xf400d000 + 0x240, 0x200);
	pr_debug("%s [%d] 0xf400d000 + 0x240:0x%x\n",
		__func__, __LINE__, READREG32(0xf400d000 + 0x240));

	WRITEREG32(0xf400d000 + 0x240, 0x200);
	pr_debug("%s [%d] 0xf400d000 + 0x240:0x%x\n",
		__func__, __LINE__, READREG32(0xf400d000 + 0x240));

	WRITEREG32(0xf400d000 + 0x244, pwm_cycle);
	pr_debug("%s [%d] 0xf400d000 + 0x244:0x%x\n",
		__func__, __LINE__, READREG32(0xf400d000 + 0x244));

	WRITEREG32(0xf400d000 + 0x248, 0x001);
	pr_debug("%s [%d] 0xf400d000 + 0x248:0x%x\n",
		__func__, __LINE__, READREG32(0xf400d000 + 0x248));

	WRITEREG32(0xf400d000 + 0x24c, 0x20000);
	pr_debug("%s [%d] 0xf400d000 + 0x24c:0x%x\n",
		__func__, __LINE__, READREG32(0xf400d000 + 0x24c));
}
...
static void tcon_dev_init(vo_device_t *vo_dev)
{
	...
	/* pwm cycle: 0 -100 */
	/* doss_set_bl_pwm_cycle(doss_index, 10); */
...
/* lvds 1280x800 LCD test */
static const struct drm_display_mode tianma_tm070jdhg30_mode = {
	/* dclk_freq */
	.clock = 66000000,
	/* width */
	.hdisplay = 1280,
	...
	.hsync_end = 1280 + 50 + 100,
	/* htotal = hdisplay + hfp + hsw + hbp */
	.htotal = 1280 + 50 + 100 + 5,
	/* height */
	.vdisplay = 800,
