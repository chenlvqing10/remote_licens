...
void drm_fbdev_cma_destory_mode(struct drm_fbdev_cma *fbdev_cma)
{
	if (fbdev_cma) {
		struct drm_device *dev = fbdev_cma->fb_helper.dev;

		drm_modeset_lock_all(dev);
		drm_fb_helper_destory_fbdev_mode(&fbdev_cma->fb_helper);
		drm_modeset_unlock_all(dev);
	}
}
EXPORT_SYMBOL_GPL(drm_fbdev_cma_destory_mode);
