...
static int pcf8563_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *tm)
{
	...
	dev_dbg(&client->dev,
		"%s: tm is mins=%d, hours=%d, mday=%d, wday=%d, enabled=%d, pending=%d\n",
		__func__,
		tm->time.tm_min, tm->time.tm_hour, tm->time.tm_mday,
		tm->time.tm_wday, tm->enabled, tm->pending);
				...
				const struct i2c_device_id *id)
{
...
#ifdef ARCH_LOMBO_N7V1
	csp_rtc_set_clk_src(VA_RTC_RTC_CLK_CTRL, 1);
#endif
	...
	if (err) {
		dev_err(&client->dev, "%s: read alarm error\n", __func__);
		return err;
	}
	if (alm_pending)
	...
	err = pcf8563_write_block_data(client, PCF8563_REG_ST2, 1, &buf);
#endif
	/* Set timer to lowest frequency to save power */
	...
	/* init rtc time when it is invalid */
	pcf8563_get_datetime(client, &rtc_tm);
	if (rtc_valid_tm(&rtc_tm)) {
		...
		err = pcf8563_set_datetime(client, &rtc_tm);
		if (err) {
			dev_err(&client->dev, "%s: set time error\n", __func__);
			return err;
		}
