		...
		unsigned long parent_rate)
{
	...
	if (is_audio_divider_clk(name) || is_audio_module_clk(name)) {
		pr_debug("%s(%d) to check: clk(%s) is audio clk\n",
				...
				unsigned long *prate)
{
	...
	if (is_audio_module_clk(name)) {
		pr_debug("%s(%d): audio clk(%s)\n", __func__, __LINE__, name);
		return audio_clk_round_rate((char *)name, rate);
	}
#endif
	...
	if (ret > rate) { /* rate too small to be set */
		pr_err("%s(%d) err: clk(%s) rate(%d) too small!\n",
			__func__, __LINE__, name, (int)rate);
		return -EINVAL; /* abort clk_set_rate */
	}
	return ret;
#else
				...
				unsigned long parent_rate)
{
	...
	if (is_audio_module_clk(name)) {
		pr_debug("%s(%d): audio clk(%s)\n", __func__, __LINE__, name);
		return audio_clk_set_rate(rate, name);
	}
#endif
