...
static unsigned int streaming_maxpacket = 512;
...
uvc_register_video(struct uvc_device *uvc)
{
	...
	return video_register_device(video, VFL_TYPE_GRABBER, 5);
}
...
uvc_copy_descriptors(struct uvc_device *uvc, enum usb_device_speed speed)
{
	 ...
	 * uvc_control_ep (maybe not use)
	 * uvc_control_cs_ep (maybe not use)
	 * uvc_ss_control_comp (for SS only) (maybe not use)
