		     ...
		     const struct usb_device_id *id)
{
	...
	dev_dbg(&intf->dev, "%s %d, vid: 0x%x, pid: 0x%x, interface: 0x%x\n",
		__func__, __LINE__, le16_to_cpu(usb_dev->descriptor.idVendor),
		le16_to_cpu(usb_dev->descriptor.idProduct),
		desc->bInterfaceNumber);

	if (detected_neoway_product(usb_dev) &&
		(desc->bInterfaceNumber == 0 || desc->bInterfaceNumber == 1)) {
		dev_info(&intf->dev, "Neoway quirk, skipping interface 0x%x\n",
			desc->bInterfaceNumber);
		return -ENODEV;
	}
#endif
