...
#include <cutils/asm/socket.h>

#include <linux/input.h>

#include "utility.h"
#include "ez_ptrace.h"
#include <linux/exec_elf.h>
...
void dump_crash_banner(int tfd, unsigned pid, unsigned tid, int sig)
{
	...
	sprintf(tid_name, "/proc/%d/task/%d/comm", pid,tid);
    fp_tid = fopen(tid_name, "r");
    if(fp) {
        y = fgets(tid_name, 1024, fp_tid);
        fclose(fp_tid);
    }
    ...
    _LOG(tfd, false,
         "*** *** *** *** *** *** *** *** *** *** *** *** *** *** *** ***\n");
    dump_build_info(tfd);
    _LOG(tfd, false, "pid: %d, tid: %d  >>> %s <<< thread: %s\n",
         pid, tid, x ? x : "UNKNOWN",y?y: "UNKNOWN");
...
static void handle_crashing_process(int fd)
{
    ...
    XLOG("BOOM: pid=%d uid=%d gid=%d tid=%d name:%s\n", cr.pid, cr.uid, cr.gid, tid, data);
...
int main(int argc, char **argv)
{
    ...
    s = socket_local_server("virgo:debuggerd", 
            ANDROID_SOCKET_NAMESPACE_ABSTRACT, 1/*SOCK_STREAM*/);
    if(s < 0) return -1;
