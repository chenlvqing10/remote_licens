...
static void use_help(void)
{
	...
	printf("	-h = show help\n");
	printf("example:\n");
	printf("	sta_connect -s test -p 12345678 -e 2 -v 1\n");
}

int main(int argc, char *argv[])
{
	int ret, tmp, opt, visibility;
	char *passwd_ptr = NULL, *security_ptr = NULL, *visibility_ptr = NULL;
	...
	for (;;) {
		opt = getopt(argc, argv, "s:p:e:v:h");
		if (opt < 0)
			break;
		switch (opt) {
		case 's':
	...
	if (visibility == 0) {
		ret = sta_connect_hidden(ssid_ptr, passwd_ptr, security);
		if (ret == -1) {
