...
#define  LOG_TAG		"sta_delete_all_net_conf"
#include <log/log.h>
...
int main(int argc, char *argv[])
{
	...
	ret = sta_delete_all_net_conf();
	if (ret == -1) {
		ALOGE("delete all network ID failed!\n");
		goto exit;
	} else {
		ALOGI("delete all network ID successed!\n");
		printf("all network ID have been deleted.\n");
	}
