...
#define  LOG_TAG		"sta_delete_net_conf"
#include <log/log.h>
...
{
	printf("usage:\n");
	printf("	sta_delete_net_conf [options]\n");
	printf("option = instruction:\n");
	printf("	-n = the  network ID to be deleted.\n");
	printf("	-h = show help\n");
	printf("example:\n");
	printf("	sta_delete_net_conf -n 2\n");
}
...
int main(int argc, char *argv[])
{
	...
	ret = sta_delete_net_conf(net_id);
	if (ret == -1) {
		ALOGE("delete network ID failed!\n");
		goto exit;
	} else {
		ALOGI("delete network ID successed!\n");
		printf("the deleted network ID : %d\n", net_id);
	}
