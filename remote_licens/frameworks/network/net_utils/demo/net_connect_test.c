...
static void usage(const char *argv0)
{
	...
	fprintf(stderr, " <destination>	destination, "
			"such as 192.168.1.3 or www.baidu.com\n");
}
...
int main(int argc, char **argv)
{
	...
	while (!g_to_exit) {
		...
		ret = net_is_connect(device, destination);
		if (ret) {
			if (device)
				ALOGI("%s is reachable by %s",
				      destination, device);
			else
				ALOGI("%s is reachable",
				      destination);
		} else {
			if (device)
				ALOGI("%s is unreachable by %s",
				      destination, device);
			else
				ALOGI("%s is unreachable",
				      destination);
		}
