...
{
	uint32_t *p;
	p = (uint32_t *)(buf->planes[0].map +
			 buf->planes[0].stride * y +
			 x * 4);
	*p = color;
}
...
static void drm_draw_test_pattern_default(struct framebuffer *fb)
{
	...
	if (fb->format == DRM_FORMAT_ARGB8888) {
		for (y = 0; y < h; y++) {
			for (x = 0; x < w; x++) {
				/* white margin lines */
				if (x == xm1 || x == xm2 || y == ym1 || y == ym2)
					draw_pixel(fb, x, y, MAKE_ARGB(125, 255, 255, 255));
				/* white box outlines to corners */
				else if ((x == 0 || x == w - 1) && (y < ym1 || y > ym2))
					draw_pixel(fb, x, y, MAKE_ARGB(125, 255, 255, 255));
				/* white box outlines to corners */
				...
				/* blue bar on the left */
					...
					draw_pixel(fb, x, y, MAKE_ARGB(125, 0, 0, 255));
				/* blue bar on the top */
				else if (y < ym1 && (x > xm1 && x < xm2))
					draw_pixel(fb, x, y, MAKE_ARGB(125, 0, 0, 255));
				/* red bar on the right */
				else if (x > xm2 && (y > ym1 && y < ym2))
					draw_pixel(fb, x, y, MAKE_ARGB(125, 255, 0, 0));
				/* red bar on the bottom */
				else if (y > ym2 && (x > xm1 && x < xm2))
					draw_pixel(fb, x, y, MAKE_ARGB(125, 255, 0, 0));
				/* inside the margins */
					...
					/* diagonal line */
						...
						draw_pixel(fb, x, y, MAKE_ARGB(125, r, g, b));
					}
				/* black corners */
				} else {
					draw_pixel(fb, x, y, 0);
...
static void fb_rgb_to_semiplanar_yuv(struct framebuffer *dst_fb,
...
static void drm_draw_color_bar_rgb888(struct framebuffer *buf,
				      int old_xpos,
				      int xpos,
				      int width)
{
...
static void drm_draw_color_bar_rgb565(struct framebuffer *buf,
				      int old_xpos,
				      int xpos,
				      int width)
{
...
static void drm_draw_color_bar_semiplanar_yuv(struct framebuffer *buf,
					      int old_xpos,
					      int xpos,
					      int width)
{
...
void drm_draw_color_bar(struct framebuffer *buf,
			int old_xpos,
			int xpos,
			int width)
{
