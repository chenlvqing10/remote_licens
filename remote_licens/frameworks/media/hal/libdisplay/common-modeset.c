...
static int modeset_find_crtc(int fd,
			     drmModeRes *res,
			     drmModeConnector *conn,
			     struct modeset_out *out,
			     struct modeset_out *out_list)
{
	drmModeEncoder *enc;
	unsigned int i, j;
	int32_t crtc_id;
	...
	ALOGE("cannot find suitable CRTC for connector %u",
		conn->connector_id);
	return -ENOENT;
}

static int modeset_setup_output(int fd,
				drmModeRes *res,
				drmModeConnector *conn,
				struct modeset_out *out,
				struct modeset_out *out_list)
{
	...
	if (conn->connection != DRM_MODE_CONNECTED) {
		ALOGE("ignoring unused connector %u",
			conn->connector_id);
		return -ENOENT;
	}
	...
	if (conn->count_modes == 0) {
		ALOGE("no valid mode for connector %u",
			conn->connector_id);
		return -EFAULT;
	}
	...
	if (r) {
		ALOGE("no valid crtc for connector %u",
			conn->connector_id);
		return r;
	}
...
void modeset_prepare(int fd, struct modeset_out **out_list)
{
	...
	for (i = 0; i < res->count_connectors; ++i) {
		...
		if (r) {
			if (r != -ENOENT) {
				errno = -r;
				ALOGE("cannot setup output %d for connector %u\n",
					i, conn->connector_id);
			}
			free(out);
...
void modeset_set_modes(struct modeset_out *list)
{
	for_each_output(out, list) {
		...
		ALOGI("Output %u: Connector %u, Encoder %u, CRTC %u, FB %u/%u, Mode %ux%u\n",
			out->output_id,
...
void modeset_main_loop(struct modeset_out *modeset_list, void (*flip_event)(void *))
{
	...
	while (true) {
		...
		if (r < 0) {
			ALOGE("select() failed with %s", ERRSTR);
			break;
		} else if (FD_ISSET(0, &fds)) {
			ALOGE("exit due to user-input");
			break;
	...
	for_each_output(out, modeset_list) {
		...
		ALOGE("wait for pending page-flip to complete for output %d...\n",
			out->output_id);
