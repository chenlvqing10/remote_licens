...
static pthread_mutex_t plane_mutex = PTHREAD_MUTEX_INITIALIZER;
...
int drm_open_dev_dumb(const char *node)
{
	...
	if (fd < 0) {
		ALOGE("%s %d %s", __func__, __LINE__, ERRSTR);
		return -1;
	}
	drmDropMaster(fd);
	ASSERT(!drmIsMaster(fd));

	unsigned int ignore_permit = 1;

	drmIoctl(fd, DRM_IOCTL_LOMBO_IGNORE_IOCTL_PERMIT, &ignore_permit);
...
void drm_create_dumb_fb2(int fd,
			 uint32_t width,
			 uint32_t height,
			 uint32_t format,
			 struct framebuffer *buf)
{
	...
	for (int i = 0; i < format_info->num_planes; ++i) {
		 ...
		 *         i, creq.width, creq.height,
		 *         pi->bitspp, plane->stride, plane->size);
		*/
...
uint32_t drm_plane_index_to_id(int idx)
{
	...
	if (idx < ARRAY_SIZE(reserved_plane_ids))
		plane_id = reserved_plane_ids[idx];
	else
		plane_id = -1;
...
int drm_plane_bottom(int idx)
{
	...
	pthread_mutex_lock(&plane_mutex);
	for (i = 0; i < ARRAY_SIZE(reserved_plane_ids); ++i) {
		if (reserved_plane_ids[i] > 0)
			valid_num++;
	}
	if (valid_num > 1) {
		min_idx = 0;
		min = reserved_plane_ids[min_idx];
		for (i = 1; i < valid_num; i++) {
			if (min > reserved_plane_ids[i]) {
				min = reserved_plane_ids[i];
				min_idx = i;
			}
		}
		if (idx != min_idx) {
			tmp = reserved_plane_ids[idx];
			reserved_plane_ids[idx] = min;
			reserved_plane_ids[min_idx] = tmp;
		}
	}
	pthread_mutex_unlock(&plane_mutex);
...
int drm_plane_top(int idx)
{
	...
	if (valid_num > 1) {
		max_idx = 0;
		max = reserved_plane_ids[max_idx];
		for (i = 1; i < valid_num; i++) {
			if (max < reserved_plane_ids[i]) {
				max = reserved_plane_ids[i];
				max_idx = i;
			}
		}
		if (idx != max_idx) {
			tmp = reserved_plane_ids[idx];
			reserved_plane_ids[idx] = max;
			reserved_plane_ids[max_idx] = tmp;
		}
	}
	pthread_mutex_unlock(&plane_mutex);
...
{
	ASSERT((idx >= 0) && (idx < ARRAY_SIZE(reserved_plane_ids)));
