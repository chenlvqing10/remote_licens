	...
	OMX_IN OMX_PTR paramData)
{
	...
	case OMX_IndexParamAudioPcm: {
		...
		switch (audio_params->ePCMMode) {
		case OMX_AUDIO_PCMModeLinear:
			audio_private->paudio.para->codec_id = AV_CODEC_TYPE_PCM_S16LE;
			break;
		case OMX_AUDIO_PCMModeALaw:
			audio_private->paudio.para->codec_id = AV_CODEC_TYPE_PCM_ALAW;
			break;
		case OMX_AUDIO_PCMModeMULaw:
			audio_private->paudio.para->codec_id = AV_CODEC_TYPE_PCM_MULAW;
			break;
		default:
			audio_private->paudio.para->codec_id = AV_CODEC_TYPE_PCM_S16LE;
			break;
	...
	case OMX_IndexParamAudioG729: {
		...
		*/
		audio_private->paudio.para->channels = 1;
		audio_private->paudio.para->bit_rate = 8000;
		audio_private->paudio.para->sample_rate = 8000;
		audio_private->acode = AV_CODEC_TYPE_G729;
	}
	break;
	...
	OMX_U32 dest_state)
{
	...
	if (dest_state == OMX_StateExecuting && component->state == OMX_StateIdle) {
		if (audio_private->dl_hdl == NULL) {
			...
			OSCL_LOGW("aenc has be initialized already");
		}
	}
...
OMX_ERRORTYPE aenc_component_init(lb_omx_component_t *cmp_handle)
{
	...
	cmp_handle->name = "OMX.LB.ENCODER.AUDIO";
...
void *omx_component_init = aenc_component_init;
#endif
