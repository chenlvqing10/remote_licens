...
static int bmp_data2bgr(BITMAPFILEHEADER *file_header ,BITMAPINFOHEADER *info_header,
	char *data, int len, unsigned char *bgr_buf)
{
	int ret = 0;
	unsigned long i = 0, j = 0;
	...
	memset(bmp_buf, 0, width * height);
	memcpy(bmp_buf, data + file_header->bf_off_bits,
		len - file_header->bf_off_bits);
...
static int bmp_auto_scale(struct bmp_dec_output *output)
{
	...
	struct bmp_dec_output *output)
{
	...
	memcpy(&head, input->data, sizeof(BITMAPFILEHEADER));
	memcpy(&info, input->data + sizeof(BITMAPFILEHEADER), sizeof(BITMAPINFOHEADER));
	...
	ret = bmp_data2bgr(&head, &info, input->data, input->len, bgr_buf);
	if (ret) {
		goto exit;
	}
	...
	struct bmp_dec_info *dec_info)
{
	...
	show_bmp_info_Head(&info);
#endif
	dec_info->width = info.bi_width; /* img width px */
	dec_info->height = info.bi_height; /* img height px */

	ALOGI("bmp width: %d, height: %d\n", dec_info->width, dec_info->height);
