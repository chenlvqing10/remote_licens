...
static int png_auto_scale(struct png_dec_output *output)
{
...
static void png_read_data_cb(png_structp png_ptr, png_bytep data, png_size_t length)
{
	...
	{
		png_error(png_ptr,"png_read_data_cb failed");
	}
}
	...
	struct png_dec_output *output)
{
	...
	imgsource.data = input->data;
	imgsource.size = input->len;
	imgsource.offset = 0;
	/* define our own callback function for I/O instead of reading from a file */
	png_set_read_fn(png, &imgsource, png_read_data_cb);
	png_read_png(png, info,
		PNG_TRANSFORM_EXPAND | PNG_TRANSFORM_STRIP_ALPHA, NULL);
